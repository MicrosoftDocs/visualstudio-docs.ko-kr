...
이러한 항목을 수동으로 수정 하거나, 메서드 호출 옆에 표시 되는 전구를 마우스로 가리키고, 제안 된 변경 내용을 선택 하 여 Visual Studio에서 자동으로이 작업을 수행 하도록 선택할 수 있습니다. 예:

![CA1836에 대 한 코드 수정-스트림 기반 클래스에서 ReadAsync/WriteAsync 메서드의 메모리 기반 오버 로드를 선호 합니다.](media/ca1836-codefix.png)
