...
이 코드는 `SyntaxGenerator` 새 코드를 생성 하는 데 매우 유용한 형식인를 사용 합니다. 코드 문제가 있는 문서에 대 한 생성기를 가져온 후에는를 호출 하 여 `ChangeToImmutableArrayEmpty` 액세스 하려는 `MemberAccessExpression` 멤버가 있는 형식을 전달 하 고 멤버의 이름을 문자열로 전달 합니다.
