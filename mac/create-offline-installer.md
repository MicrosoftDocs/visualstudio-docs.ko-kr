---
title: 오프라인 설치 관리자 만들기
description: ''
author: heiligerdankgesang
ms.author: dominicn
ms.date: 11/06/2018
ms.technology: vs-ide-install
ms.assetid: 5CA4B8C1-2D01-46EA-A6B2-8573B324509B
ms.openlocfilehash: 3f5304364c3c9ea39242efcae7f0646a0db909ff
ms.sourcegitcommit: 2975d722a6d6e45f7887b05e9b526e91cffb0bcf
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 03/20/2020
ms.locfileid: "74983993"
---
# <a name="create-an-offline-installer-for-visual-studio-for-mac"></a>Mac용 Visual Studio 오프라인 설치 관리자 만들기

> [!IMPORTANT]
> 오프라인 설치 관리자 기능은 이후 버전의 Mac용 Visual Studio에 대해 예정되어 있습니다.

## <a name="see-also"></a>참고 항목

- [Visual Studio의 오프라인 설치 만들기(Windows에서)](/visualstudio/install/create-an-offline-installation-of-visual-studio)
