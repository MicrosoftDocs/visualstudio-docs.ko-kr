...
description: 리터럴 문자열은 지역화할 수 있는 매개 변수 또는 지역화 가능한 속성에 대 한 인수로 전달 되 고, 문자열에는 Microsoft 맞춤법 검사 라이브러리에서 인식 하지 못하는 단어가 하나 이상 포함 되어 있습니다.
ms.date: 03/28/2018
ms.topic: reference
f1_keywords:
