...
description: Dsl 정의 파일에서 dsl (도메인별 언어)을 정의 하는 방법 및 DSL 솔루션의 프로그램 코드 대부분이이 파일에서 생성 되는 방법에 대해 알아봅니다.
...
## <a name="multiplicities"></a>복합성
 복합성 도메인 관계에서 동일한 역할을 가질 수 있는 요소 수를 지정 합니다. 이 예제에서 카탈로그 역할에 대 한 일 대 다 (0 ... \* ) 다중성 설정은 **게시자** 도메인  클래스의 모든 인스턴스가 **PublisherCatalog** 관계 링크를 제공할 수 있도록 지정 합니다.
