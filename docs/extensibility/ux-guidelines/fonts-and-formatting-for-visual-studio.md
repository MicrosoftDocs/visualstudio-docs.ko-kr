...
ms.locfileid: "78409081"
---
# <a name="fonts-and-formatting-for-visual-studio"></a>Visual Studio의 글꼴 및 서식
...
### <a name="accessing-the-environment-font"></a>환경 글꼴 액세스
 네이티브 또는 WinForms 코드에서 `SID_SUIHostLocale` 서비스의 인터페이스를 쿼리 한 후 `IUIHostLocale::GetDialogFont` 메서드를 호출 하 여 환경 글꼴에 액세스할 수 있습니다.

 Windows Presentation Foundation (WPF)의 경우 WPF의 `Window` 클래스 대신 셸에서의 `DialogWindow` 클래스에서 대화 상자 창 클래스를 파생 시킵니다.
...
```
 ...
 `Microsoft.VisualStudio.Shell.11.0`를 현재 버전의 MPF dll로 바꿉니다.

 대화 상자를 표시 하려면 `ShowDialog()`를 통해 클래스에서 "`ShowModal()`"를 호출 합니다. `ShowModal()`는 셸에서 올바른 모달 상태를 설정 하 고, 대화 상자가 부모 창에서 가운데 맞춤 되도록 합니다.
...
```
 ...
 `ShowModal` 부울 값을 반환 하나요? (nullable 부울) `DialogResult`(필요한 경우 사용할 수 있음). 대화 상자가 **정상**으로 닫히면 반환 값은 true입니다.

 대화가 아니고 Win32/WinForms 부모 창의 팝업 창이 나 WPF 자식 창과 같은 자체 `HwndSource`에 호스팅된 WPF UI를 표시 해야 하는 경우에는 `FontFamily`를 설정 하 고 WPF 요소의 루트 요소에 `FontSize` 해야 합니다. (셸은 주 창에서 속성을 설정 하지만 `HWND`이전에는 상속 되지 않습니다.) Shell은 다음과 같이 속성을 바인딩할 수 있는 리소스를 제공 합니다.
...
### <a name="BKMK_Formatting"></a>서식 지정 (배율/굵은 글꼴) 참조
 일부 대화 상자에서는 특정 텍스트를 굵게 하거나 환경 글꼴 이외의 크기를 설정 해야 합니다. 이전에는 환경 글꼴 보다 큰 글꼴이 "`environment font +2`" 또는 이와 비슷하게 코딩 되었습니다. 제공 된 코드 조각을 사용 하 여 높은 DPI 모니터를 지원 하 고 표시 텍스트가 항상 올바른 크기와 무게 (예: 빛 또는 좋아요)로 표시 되는지 확인 합니다.
...
> 서식 지정을 적용 하기 전에 [텍스트 스타일](../../extensibility/ux-guidelines/fonts-and-formatting-for-visual-studio.md#BKMK_TextStyle)에 있는 지침을 충족 하는지 확인 합니다. * *
 ...
 여기서 "`vsui`"는 네임 스페이스 `Microsoft.VisualStudio.Shell`에 대 한 참조입니다.
...
**절차적 코드:** 여기서 `textBlock`은 이전에 정의 된 TextBlock 이며 `label` 이전에 정의 된 레이블입니다.
...
### <a name="localizable-styles"></a>지역화할 수 있는 스타일
 ...
 이 솔루션은 대화 상자의 `FontChanged` 이벤트를 연결 하는 것입니다. `FontChanged` 이벤트에서 모든 컨트롤을 탐색 하 고 해당 글꼴이 설정 되었는지 확인 합니다. 설정 되어 있으면 폼의 글꼴 및 컨트롤의 이전 글꼴 스타일에 따라 새 글꼴로 변경 합니다. 코드의 예제는 다음과 같습니다.
...
```
 ...
 이 코드를 사용 하면 폼의 글꼴이 업데이트 될 때 컨트롤의 글꼴도 업데이트 됩니다. 이 메서드는 폼의 생성자에서 호출 해야 합니다. 대화 상자에서 `IUIService`의 인스턴스를 가져오지 못해 `FontChanged` 이벤트가 발생 하지 않을 수 있기 때문입니다. `FontChanged` 후크 하면 대화 상자가 이미 열려 있는 경우에도 대화에서 새 글꼴을 동적으로 선택할 수 있습니다.
...
|||
|-|-|
|**사용:**<br /><br /> -부제목<br />-작은 및 보통 대화 상자의 제목<br /><br /> **시겠습니까**<br /><br /> -문장 사례 사용<br />-항상 사용 가중치를 사용 합니다.<br /><br /> **안 함:**<br /><br /> -굵게, 기울임꼴 또는 굵은 기울임꼴<br />-본문 텍스트에 사용<br />-도구 창에서 사용|18 pt 맑은 고딕 Semillight **로 표시 됩니다.**<br /><br /> **시각적 예제:**<br /><br /> ![200% 환경 글꼴 &#43; 의 예](../../extensibility/ux-guidelines/media/0202-b_ef200.png "0202-b_EF200")|

#### <a name="155-environment-font"></a>155% 환경 글꼴

|||
|-|-|
|**사용:**<br /><br /> -문서 웰 UI의 섹션 제목<br />-보고서<br /><br /> **Do:** 문장 사례 사용<br /><br /> **안 함:**<br /><br /> -굵게, 기울임꼴 또는 굵은 기울임꼴<br />-본문 텍스트에 사용<br />-표준 Visual Studio 컨트롤에서 사용<br />-도구 창에서 사용|**다음과 같이 표시 됩니다.** 14 pt 맑은 고딕<br /><br /> **시각적 예제:**<br /><br /> ![155% 환경 글꼴 제목의 예](../../extensibility/ux-guidelines/media/0202-c_ef155.png "0202-c_EF155")|

#### <a name="133-environment-font"></a>133% 환경 글꼴

|||
|-|-|
|**사용:**<br /><br /> -서명 대화 상자의 작은 부제목<br />-문서 웰 UI의 작은 부제목<br /><br /> **Do:** 문장 사례 사용<br /><br /> **안 함:**<br /><br /> -굵게, 기울임꼴 또는 굵은 기울임꼴<br />-본문 텍스트에 사용<br />-표준 Visual Studio 컨트롤에서 사용<br />-도구 창에서 사용|12 pt 맑은 고딕 **로 표시 됩니다.**<br /><br /> **시각적 예제:**<br /><br /> ![133% 환경 글꼴 제목의 예](../../extensibility/ux-guidelines/media/0202-d_ef133.png "0202-d_EF133")|

#### <a name="122-environment-font"></a>122% 환경 글꼴

|||
|-|-|
|**사용:**<br /><br /> -서명 대화 상자의 섹션 제목<br />-트리 뷰의 최상위 노드<br />-세로 탭 탐색<br /><br /> **Do:** 문장 사례 사용<br /><br /> **안 함:**<br /><br /> -굵게, 기울임꼴 또는 굵은 기울임꼴<br />-본문 텍스트에 사용<br />-표준 Visual Studio 컨트롤에서 사용<br />-도구 창에서 사용|**다음과 같이 표시 됩니다.** 11pt 맑은 고딕<br /><br /> **시각적 예제:**<br /><br /> ![122% 환경 글꼴 제목의 예](../../extensibility/ux-guidelines/media/0202-e_ef122.png "0202-e_EF122")|

#### <a name="environment-font--bold"></a>환경 글꼴 + 굵게

|||
|-|-|
|**사용:**<br /><br /> -서명 대화 상자의 레이블 및 부제<br />-보고서의 레이블 및 부제<br />-문서 웰 UI의 레이블 및 부제<br /><br /> **시겠습니까**<br /><br /> -문장 사례 사용<br />-굵은 두께 사용<br /><br /> **안 함:**<br /><br /> -기울임꼴 또는 굵게 기울임꼴<br />-본문 텍스트에 사용<br />-표준 Visual Studio 컨트롤에서 사용<br />-도구 창에서 사용|굵게 **표시 되는** 9 pt 맑은 고딕<br /><br /> **시각적 예제:**<br /><br /> ![환경 글꼴 &#43; 굵은 제목 예](../../extensibility/ux-guidelines/media/0202-f_efb.png "0202-f_EFB")|
