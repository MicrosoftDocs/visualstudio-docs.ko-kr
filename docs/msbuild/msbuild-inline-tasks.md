...
- `AssemblyFile` 특성은 인라인 작업 팩터리를 위치를 지정합니다. 또는 `AssemblyName` 특성을 사용하여 일반적으로 `$(MSBuildToolsPath)\Microsoft.Build.Tasks.Core.dll`에 있는 인라인 작업 팩터리 클래스의 정규화된 이름을 지정할 수 있습니다.
