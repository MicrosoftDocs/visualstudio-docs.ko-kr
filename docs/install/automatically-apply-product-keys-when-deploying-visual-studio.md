...
> [!IMPORTANT]
> Visual Studio 2022는 현재 미리 보기로 제공되며 미리 보기 기간에는 제품 키를 적용할 필요가 없는 평가판 라이선스로 Visual Studio 2022를 사용할 수 있습니다.
...
대상 컴퓨터에서 자동 모드로 `StorePID.exe` 유틸리티를 사용하여 설치된 Visual Studio 버전을 제품 키로 활성화할 수 있습니다. `StorePID.exe`는 다음 기본 위치에 Visual Studio 2017과 함께 설치되는 유틸리티 프로그램입니다.
...
대상 컴퓨터에서 자동 모드로 `StorePID.exe` 유틸리티를 사용하여 설치된 Visual Studio 버전을 제품 키로 활성화할 수 있습니다. `StorePID.exe`는 다음 기본 위치에 Visual Studio 2019와 함께 설치되는 유틸리티 프로그램입니다.
...
> [!NOTE]
> Visual Studio의 가상 인스턴스를 실행하는 경우, 로컬 AppData 폴더와 레지스트리도 가상화해야 합니다. 가상 인스턴스 문제를 해결하려면 `<Visual Studio installation directory>\Common7\IDE\DDConfigCA.exe`를 실행합니다.  
