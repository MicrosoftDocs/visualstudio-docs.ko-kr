...
| 특성 | 설명 |
|-----------------------| - |
| `dependencyType` | 필수 요소. 종속성 유형을 지정 합니다. 유효한 값은 `preprequisite` 및 `install`입니다. `install`어셈블리는 응용 프로그램의 일부로 설치 됩니다 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] . `prerequisite`어셈블리가 GAC (전역 어셈블리 캐시)에 있어야 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] 응용 프로그램을 설치할 수 있습니다. |
