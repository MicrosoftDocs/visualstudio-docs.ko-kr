---
title: Visual Studio | 애플리케이션 패턴 Microsoft Docs
description: Visual Studio 새 기능에 대한 창 사용 패턴을 포함하여 문서 창, 도구 창 및 모뎀 없는 대화 상자 간의 차이점에 대해 알아봅니다.
...
Visual Studio 사용되는 두 가지 주요 창 유형은 문서 편집기 및 도구 창입니다. 드물지만 가능한 경우는 대규모 모뎀 없는 대화입니다. 이러한 패턴은 모두 셸에서 모더리스이지만 기본적으로 다릅니다. 이 섹션에서는 문서 창, 도구 창 및 모뎀 없는 대화 상자 간의 차이점에 대해 설명합니다. 모달 대화 패턴은 대화 상자 에서 [다룹니다.](../../extensibility/ux-guidelines/application-patterns-for-visual-studio.md#BKMK_Dialogs)

### <a name="comparing-window-usage-patterns"></a>창 사용 패턴 비교
**문서 창은** 거의 항상 문서 웰 내에 표시됩니다. 이렇게 하면 문서 편집기에 보조 도구 창을 정렬하는 "가운데 단계"가 제공됩니다.

**도구 창은** IDE의 가장자리에 대해 축소된 별도의 작은 창으로 표시되는 경우가 가장 많습니다. 이 값은 표시, 숨김 또는 자동 숨김일 수 있습니다. 그러나 창에서 **창/도킹** 속성을 선택 취소하여 문서 웰 내에 도구 창이 표시되는 경우도 있습니다. 따라서 더 많은 부동산이 생성되지만 일반적인 디자인 결정도 있습니다. Visual Studio 통합하려고 할 때 기능에서 도구 창 또는 문서 창을 표시할지 여부를 결정해야 합니다.

Visual Studio **모뎀 없는 대화는** 권장되지 않습니다. 대부분의 모뎀 없는 대화 상자는 기본적으로 부동 도구 창이며 이러한 방식으로 구현되어야 합니다. 셸 측면에 도킹된 일반 도구 창의 크기가 너무 제한적인 경우 모뎀 없는 대화 상자가 허용됩니다. 사용자가 대화 상자를 보조 모니터로 이동할 가능성이 있는 경우에도 허용됩니다.

어떤 컨테이너 형식이 필요한지 신중하게 고려해야 합니다. UI 디자인에 대한 일반적인 사용 패턴 고려 사항은 다음 표에 있습니다.

||문서 창|도구 창|모뎀 없는 대화 상자|
|-|---------------------|-----------------|---------------------|
| **Position** | 항상 문서 웰 내에 배치되며 IDE의 가장자리 주위에 도킹되지 않습니다. 기본 셸에서 별도로 부동되도록 "끌어올" 수 있습니다. | 일반적으로 IDE의 가장자리를 탭으로 고정하지만 부동, 자동 숨김(고정 해제) 또는 문서 웰 내에 도킹하도록 사용자 지정할 수 있습니다.|IDE와 분리된 큰 부동 창입니다. |
| **커밋 모델** | *지연된 커밋*<br /><br /> 문서에 데이터를 저장하려면 사용자가 **파일 &gt; 저장,** 다른 이름으로 **저장** 또는 **모두 저장** 명령을 실행해야 합니다. 문서 창에는 저장 명령 중 하나로 커밋된 후 "비정상" 안에 있는 데이터의 개념이 있습니다. 문서 창을 닫으면 모든 내용이 디스크에 저장되거나 손실됩니다. | *즉시 커밋*<br /><br /> 저장 모델이 없습니다. 파일을 편집하는 데 도움이 되는 검사기 도구 창의 경우 파일이 활성 편집기 또는 디자이너에서 열려 있어야 하며 편집기 또는 디자이너가 저장을 소유해야 합니다. | *지연되거나 즉시 커밋*<br /><br /> 대부분의 경우 대규모 모뎀 없는 대화에는 변경 내용을 커밋하는 작업이 필요하며 대화 세션 내에서 변경된 내용을 롤백하는 "취소" 작업이 허용됩니다.  이렇게 하면 모뎀 없는 대화 상자가 해당 도구 창의 도구 창과 항상 즉시 커밋 모델이 구분됩니다. |
| **표시 유형** | *열기/만들기(파일) 및 닫기*<br /><br /> 문서 창을 여는 작업은 기존 문서를 열거나 템플릿을 사용하여 새 문서를 만드는 방법을 통해 수행됩니다. \<specific editor>"Open" 명령이 없습니다. | *숨기기 및 표시*<br /><br /> 단일 인스턴스 도구 창을 숨기거나 표시할 수 있습니다. 도구 창 내의 내용과 상태는 보기에 있든 숨겨지든 관계없이 유지됩니다. 다중 인스턴스 도구 창을 닫고 숨길 수 있습니다. 다중 인스턴스 도구 창이 닫혀 있으면 도구 창 내의 콘텐츠와 상태가 삭제됩니다. | *명령에서 시작*<br /><br /> 대화 상자는 작업 기반 명령에서 시작됩니다. |
| **인스턴스** | *다중 인스턴스*<br /><br /> 여러 편집기를 동시에 열고 다른 파일을 편집할 수 있는 반면, 일부 편집기에서는 창 **&gt; 새 창** 명령을 사용하여 동일한 파일을 두 개 이상의 편집기에서 열 수 있습니다.<br /><br /> 단일 편집기에서 하나 또는 여러 파일을 동시에 편집할 수 있습니다(프로젝트 디자이너). | *단일 또는 다중 인스턴스*<br /><br /> 콘텐츠는 속성 브라우저에서와 같이 컨텍스트를 반영하거나 포커스/컨텍스트를 다른 창에 푸시하도록 변경됩니다(작업 목록, 솔루션 탐색기).<br /><br /> 단일 인스턴스 및 다중 인스턴스 도구 창은 모두 활성 문서 창과 연결되어야 합니다. | *단일 인스턴스* |
| **예** | **코드 편집기처럼 텍스트** 편집기<br /><br /> 폼 디자이너 또는 모델링 **표면과** 같은 디자인 표면<br /><br /> 매니페스트 디자이너와 같은 **대화 상자와 유사한 레이아웃 제어** | **솔루션 탐색기** 솔루션 및 솔루션에 포함된 프로젝트를 제공합니다.<br /><br /> **서버 탐색기** 사용자가 창에서 열도록 선택한 서버 및 데이터 연결의 계층적 보기를 제공합니다. 쿼리와 같이 데이터베이스 계층에서 개체를 열면 문서 창이 열리고 사용자가 쿼리를 편집할 수 있습니다.<br /><br /> **속성 브라우저는** 문서 창 또는 다른 도구 창에서 선택한 개체에 대한 속성을 표시합니다. 속성은 계층적 그리드 보기 또는 복잡한 대화 상자와 같은 컨트롤에 표시되며 사용자가 해당 속성에 대한 값을 설정할 수 있도록 합니다. | |

## <a name="tool-windows"></a><a name="BKMK_ToolWindows"></a> 도구 창

### <a name="overview"></a>개요
도구 창은 문서 창에서 발생하는 사용자의 작업을 지원합니다. Visual Studio 제공하고 조작할 수 있는 기본 루트 개체를 나타내는 계층을 표시하는 데 사용할 수 있습니다.

IDE에서 새 도구 창을 고려할 때 작성자는 다음을 수행해야 합니다.

- 작업에 적합한 기존 도구 창을 사용하고 유사한 기능을 사용하여 새 창을 만들지 않습니다. 새 도구 창은 유사한 창에 통합할 수 없는 매우 다른 "도구" 또는 기능을 제공하거나 기존 창을 피벗 허브로 전환하는 경우에만 만들어야 합니다.

- 필요한 경우 도구 창 맨 위에 있는 표준 명령 모음을 사용합니다.

- 컨트롤 프레젠테이션 및 키보드 탐색을 위해 다른 도구 창에 이미 있는 패턴과 일치해야 합니다.

- 다른 도구 창에서 컨트롤 프레젠테이션과 일치해야 합니다.

- 가능한 경우 부모 문서가 활성화될 때만 표시되도록 문서별 도구 창을 자동으로 표시합니다.

- 키보드(지원 화살표 키)로 창 콘텐츠를 탐색할 수 있는지 확인합니다.

#### <a name="tool-window-states"></a>도구 창 상태
Visual Studio 도구 창의 상태는 서로 다르며, 그 중 일부는 사용자가 활성화됩니다(예: 자동 숨기기 기능). 자동 표시와 같은 다른 상태에서는 도구 창이 올바른 컨텍스트에 표시되고 필요하지 않은 경우 숨겨지도록 허용합니다. 총 5개의 도구 창 상태가 있습니다.

- **도킹/고정된** 도구 창은 문서 영역의 네 면에 연결할 수 있습니다. 도구 창 제목 표시줄에 압정 아이콘이 나타납니다. 도구 창은 셸 및 기타 도구 창의 가장자리를 따라 가로 또는 세로로 도킹할 수 있으며 탭으로 연결될 수도 있습니다.

- **자동 숨겨진** 도구 창이 고정 해제됩니다. 창이 보이지 않게 슬라이드되어 문서 영역 가장자리에 탭(도구 창의 이름 및 아이콘)이 남을 수 있습니다. 사용자가 탭 위로 마우스를 가져가면 도구 창이 슬라이드 아웃됩니다.

- **자동 표시** 도구 창은 편집기 같은 다른 UI 부분이 시작되거나 포커스를 얻을 때 자동으로 표시됩니다.

- **부동** 도구 창은 IDE 바깥쪽을 가리킵니다. 다중 모니터 구성에 유용합니다.

- 탭 문서 도구 **창은** 문서 웰 내에 도킹할 수 있습니다. 이 방법은 개체 브라우저와 같이 프레임 가장자리에 도킹하는 것보다 더 많은 부동산이 필요한 대형 도구 창에 유용합니다.
...
| --- | --- | --- |
| 탭 컨트롤 | 대화 상자 페이지를 관련 집합으로 논리적으로 그룹화<br /><br />대화 상자에 있는 관련 컨트롤의 페이지 수 (또는 대화 상자에서 한 행에 맞는 탭 수) 보다 작은 경우에 유용 합니다.<br /><br />탭 레이블은 짧아야 합니다. 콘텐츠를 쉽게 식별할 수 있는 단어 하나 또는 두 개<br /><br />일반적인 시스템 대화 스타일<br /><br />예: **항목 &gt; 속성 파일 탐색기** | 설명이 포함된 짧은 레이블을 만드는 것은 어려울 수 있습니다.<br /><br />일반적으로 한 대화 상자에서 탭 5개를 넘지 않습니다.<br /><br />한 행에 대한 탭이 너무 많은 경우 적합하지 않습니다(대체 계층화 기술 사용).<br /><br />extensible이 아닙니다. |
| 사이드바 탐색 | 탭보다 더 많은 범주를 수용할 수 있는 간단한 전환 디바이스<br /><br />범주의 플랫 목록(계층 구조 없음)<br /><br />확장 가능<br /><br />예제: **사용자 지정... &gt; 명령 추가** | 그룹이 3개 미만인 경우 가로 공간을 잘 사용하지 않습니다.<br /><br />태스크가 드롭다운에 더 적합할 수 있습니다. |
| 트리 컨트롤 | 무제한 범주 허용<br /><br />범주의 그룹화 및/또는 계층 구조 허용<br /><br />확장 가능<br /><br />예제: **도구 &gt; 옵션** | 중첩 계층이 너무 많으면 가로 스크롤이 과도하게 발생할 수 있습니다.<br /><br />Visual Studio 트리 뷰가 과도하게 |
| 마법사 | 작업 기반 순차적 단계를 통해 사용자를 안내하여 작업 완료에 도움이 됩니다. 마법사는 상위 수준 작업을 나타내고 개별 패널은 전체 작업을 수행하는 데 필요한 하위 작업을 나타냅니다.<br /><br />사용자가 작업을 완료하기 위해 여러 편집기 및 도구 창을 사용해야 하는 경우처럼 태스크가 Ui 경계를 넘을 때 유용합니다.<br /><br />태스크에 분기가 필요한 경우에 유용합니다.<br /><br />태스크에 단계 간의 의존성이 포함된 경우에 유용합니다.<br /><br />하나의 의사 결정 포크가 있는 여러 유사한 작업을 하나의 대화 상자에 표시하여 서로 다른 유사한 대화 상자의 수를 줄일 수 있는 경우에 유용합니다. | 순차 워크플로가 필요하지 않은 작업에는 적합하지 않습니다.<br /><br />단계가 너무 많은 마법사로 인해 사용자가 과부전하고 혼동될 수 있습니다.<br /><br />마법사에는 기본적으로 제한된 화면 부동산이 있습니다. |

##### <a name="hallways-or-dashboards"></a>홀로 또는 대시보드
홀로와 대시보드는 다른 대화 상자 및 창에 대한 시작 지점 역할을 하는 대화 상자 또는 패널입니다. 잘 디자인된 "홀도"는 가장 일반적인 옵션, 명령 및 설정만 즉시 표면화하여 사용자가 일반적인 작업을 쉽게 수행할 수 있도록 합니다. 실제 회차가 그 뒤의 회의실에 액세스할 수 있는 출입구를 제공하는 것처럼, 여기서는 덜 일반적인 UI가 주 회차에서 액세스할 수 있는 관련 기능의 별도의 "방"(종종 다른 대화 상자)으로 수집됩니다.

또는 덜 일반적인 기능을 별도의 위치로 리팩터링하는 대신 단일 컬렉션에서 사용 가능한 모든 기능을 제공하는 UI는 단순히 대시보드입니다.

![Outlook에서 추가 UI를 노출하기 위한 Hallway 개념](../../extensibility/ux-guidelines/media/0704-08_hallway.png "0704-08_Hallway")<br />Outlook에서 추가 UI를 노출하기 위한 Hallway 개념

##### <a name="adaptive-ui"></a>적응형 UI
사용량 또는 사용자의 자체 보고 환경을 기반으로 UI를 표시하거나 숨기는 것은 다른 부분을 숨기면서 필요한 UI를 표시하는 또 다른 방법입니다. UI를 표시하거나 숨길 시기를 결정하는 알고리즘은 어려울 수 있으며 일부 사례 집합에 대해 규칙이 항상 잘못될 수 있기 때문에 Visual Studio 권장되지 않습니다.

## <a name="projects"></a><a name="BKMK_Projects"></a> 프로젝트

### <a name="projects-in-the-solution-explorer"></a>솔루션 탐색기 프로젝트
대부분의 프로젝트는 참조 기반, 디렉터리 기반 또는 혼합으로 분류됩니다. 세 가지 유형의 프로젝트가 모두 솔루션 탐색기 동시에 지원됩니다. 프로젝트 작업에서 사용자 환경의 루트는 이 창 내에서 발생합니다. 다른 프로젝트 노드가 참조, 디렉터리 또는 혼합 모드 형식 프로젝트이지만 프로젝트별 사용자 패턴으로 분기하기 전에 시작점으로 적용해야 하는 일반적인 상호 작용 패턴이 있습니다.

프로젝트는 항상 다음을 수행해야 합니다.

- 프로젝트 폴더를 추가하여 프로젝트 콘텐츠를 구성하는 기능 지원

- 프로젝트 지속성을 위한 일관된 모델 유지 관리

또한 프로젝트는 다음을 위해 일관된 상호 작용 모델을 유지해야 합니다.
...
프로젝트는 일반적으로 자신을 참조 기반(스토리지의 프로젝트 항목에 대한 참조만 유지할 수 있음), 디렉터리 기반(프로젝트의 계층 구조 내에 물리적으로 저장된 프로젝트 항목만 유지할 수 있음) 또는 혼합(참조 또는 물리적 항목을 유지할 수 있음)으로 분류합니다. IDE는 **솔루션 탐색기** 내에서 동시에 세 가지 유형의 프로젝트를 모두 수용합니다.

끌어서 놓기 관점에서 다음 특성은 **솔루션 탐색기** 내의 각 프로젝트 형식에 적용되어야 합니다.

- **참조 기반 프로젝트:** 핵심은 프로젝트가 스토리지의 항목에 대한 참조를 끌고 있다는 것입니다. 참조 기반 프로젝트가 이동 작업의 소스 역할을 하는 경우 프로젝트에서 항목에 대한 참조만 제거해야 합니다. 항목이 실제로 하드 드라이브에서 삭제 되지 않아야 합니다. 참조 기반 프로젝트가 이동(또는 복사) 작업의 대상 역할을 하는 경우 항목의 프라이빗 복사본을 만들지 않고 원래 원본 항목에 대한 참조를 추가해야 합니다.

- **디렉터리 기반 프로젝트:** 끌어서 놓기 관점에서 프로젝트는 참조가 아닌 실제 항목을 끌어서 놓습니다. 디렉터리 기반 프로젝트가 이동 작업의 원본 역할을 하는 경우 하드 드라이브에서 물리적 항목을 삭제하고 프로젝트에서 제거해야 합니다. 디렉터리 기반 프로젝트가 이동(또는 복사) 작업의 대상 역할을 하는 경우 대상 위치에 원본 항목의 복사본을 만들어야 합니다.

- **혼합 대상 프로젝트:** 끌어서 놓기 관점에서 이 프로젝트 형식의 동작은 끌어서 놓는 항목의 특성(스토리지의 항목에 대한 참조 또는 항목 자체)을 기반으로 합니다. 참조 및 물리적 항목에 대한 올바른 동작은 위에서 설명합니다.

**솔루션 탐색기** 한 가지 형식의 프로젝트만 있는 경우 끌어서 놓기 작업은 간단합니다. 각 프로젝트 시스템에는 고유한 끌어서 놓기 동작을 정의하는 기능이 있으므로 예측 가능한 사용자 환경을 보장하기 위해 특정 지침(Windows 탐색기 끌어서 놓기 동작 기반)을 따라야 합니다.

- **솔루션 탐색기** 수정되지 않은 끌기 작업(Ctrl 키와 Shift 키를 모두 누르지 않은 경우)은 이동 작업을 발생시켜야 합니다.

- Shift 끌기 작업으로 인해 이동 작업도 발생합니다.

- Ctrl 끌기 작업으로 인해 복사 작업이 발생합니다.

- 참조 기반 및 혼합 프로젝트 시스템은 소스 항목에 링크(또는 참조)를 추가하는 개념을 지원합니다. 이러한 프로젝트가 끌어서 놓기 작업의 대상인 **경우(Ctrl + Shift 키를** 누른 경우) 항목에 대한 참조가 프로젝트에 추가됩니다.

모든 끌어서 놓기 작업이 참조 기반, 디렉터리 기반 및 혼합 프로젝트의 조합에서 합리적인 것은 아닙니다. 특히 디렉터리 기반 원본 프로젝트와 참조 기반 대상 프로젝트 간에 이동 작업을 허용하는 것은 문제가 됩니다. 원본 디렉터리 기반 프로젝트는 이동 완료 시 원본 항목을 삭제해야 하기 때문입니다. 그러면 대상 참조 기반 프로젝트가 삭제된 항목에 대한 참조로 끝납니다.

또한 대상 참조 기반 프로젝트에서 소스 항목의 독립적인 복사본을 만들지 않아야 하므로 이러한 형식의 프로젝트 간에 복사 작업을 허용하는 것이 잘못되었습니다. 마찬가지로 디렉터리 기반 프로젝트에서 참조를 유지할 수 없으므로 Ctrl+ Shift 키를 디렉터리 기반 대상 프로젝트로 끌 수 없습니다. 끌어서 놓기 작업이 지원되지 않는 경우 IDE는 놓기를 허용되지 않고 사용자에게 놓지 않음 커서를 표시해야 합니다(아래 포인터 표에 표시됨).

끌어서 놓기 동작을 제대로 구현하려면 끌기의 원본 프로젝트가 대상 프로젝트에 특성을 전달해야 합니다. (예를 들어 참조 또는 디렉터리 기반인가요?) 이 정보는 원본에서 제공하는 클립보드 형식으로 표시됩니다. 끌기(또는 클립보드 복사 작업)의 원본으로 프로젝트는 `CF_VSREFPROJECTITEMS` `CF_VSSTGPROJECTITEMS` 참조 기반인지 디렉터리 기반인지에 따라 각각 또는 를 제공해야 합니다. 두 형식 모두 동일한 데이터 콘텐츠를 가지며, 이는 `CF_HDROP` 파일 이름이 아닌 문자열 목록이 두 번 `NULL` 종료된 문자열 `Projref` 목록(또는 적절하게 반환됨)이라는 점을 제외하고 Windows `IVsSolution::GetProjrefOfItem` `::GetProjrefOfProject` 형식과 비슷합니다.

끌어서 놓기 작업의 `CF_VSREFPROJECTITEMS` 정확한 처리는 대상 프로젝트 및 원본 프로젝트의 특성에 따라 달라지지만, 끌어서 놓기(또는 클립보드 붙여넣기 작업)의 대상으로 프로젝트가 및 를 모두 허용해야 `CF_VSSTGPROJECTITEMS` 합니다. 소스 프로젝트는 또는 를 제공하는지 여부에 따라 특성을 `CF_VSREFPROJECTITEMS` `CF_VSSTGPROJECTITEMS` 선언합니다. 놓기의 대상은 고유한 특성을 이해하므로 이동, 복사 또는 링크를 수행할지 여부를 결정하는 데 충분한 정보가 있습니다. 또한 사용자는 Ctrl, Shift 또는 Ctrl 키와 Shift 키를 모두 눌러 수행해야 하는 끌어서 놓기 작업을 수정합니다. 놓기 대상은 및 메서드에서 미리 수행될 작업을 올바르게 나타내는 것이 `DragEnter` `DragOver` 중요합니다. **솔루션 탐색기** 원본 프로젝트와 대상 프로젝트가 동일한 프로젝트인지 여부를 자동으로 파악합니다.

Visual Studio 인스턴스(예: devenv.exe 인스턴스 간에 프로젝트 항목 끌기)는 특별히 지원되지 않습니다. **또한 솔루션 탐색기** 이 기능을 직접 사용하지 않도록 설정합니다.

사용자는 항상 항목을 선택하고, 대상 위치로 끌어서 놓고, 항목을 삭제하기 전에 나타나는 다음 마우스 포인터를 관찰하여 끌어서 놓기 작업의 효과를 확인할 수 있어야 합니다.

| 마우스 포인터 | 명령 | 설명 |
| :---: | --- | --- |
| ![마우스 "끌어 놓기 없음" 아이콘](../../extensibility/ux-guidelines/media/0706-01_mousenodrop.png "0706-01_MouseNoDrop") | 삭제 안 했습니다. | 항목을 지정된 위치에 삭제할 수 없습니다. |
| ![마우스 "복사" 아이콘](../../extensibility/ux-guidelines/media/0706-02_mousecopy.png "0706-02_MouseCopy") | 복사 | 항목이 대상 위치에 복사됩니다. |
...
#### <a name="reference-based-projects"></a>참조 기반 프로젝트
 다음 표에는 원본 항목의 특성 및 참조 기반 대상 프로젝트에 대해 누른 한정자 키에 따라 수행해야 하는 끌어서 놓기(잘라내기/복사/붙여넣기) 작업이 요약되어 있습니다.

| 한정자 | 범주 | 원본 항목: 참조/링크 | 원본 항목: 물리적 항목 또는 파일 시스템( `CF_HDROP` ) |
| --- | --- | --- | --- |
| 한정자 없음 | 작업 | 이동 | 링크 |
| 한정자 없음 | 대상 | 원래 항목에 대한 참조 추가 | 원래 항목에 대한 참조 추가 |
| 한정자 없음 | 원본 | 원래 항목에 대한 참조를 삭제합니다. | 원래 항목을 유지합니다. |
| 한정자 없음 | 결과 | `DROPEFFECT_MOVE` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | `DROPEFFECT_LINK` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. |
| Shift+끌기 | 작업 | 이동 | 삭제 안 했습니다. |
| Shift+끌기 | 대상 | 원래 항목에 대한 참조 추가 | 삭제 안 했습니다. |
| Shift+끌기 | 원본 | 원래 항목에 대한 참조를 삭제합니다. | 삭제 안 했습니다. |
| Shift+끌기 | 결과 | `DROPEFFECT_MOVE` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | 삭제 안 했습니다. |
| Ctrl+끌기 | 작업 | 복사 | 삭제 안 했습니다. |
| Ctrl+끌기 | 대상 | 원래 항목에 대한 참조 추가 | 삭제 안 했습니다. |
| Ctrl+끌기 | 원본 | 원래 항목에 대한 참조를 유지합니다. | 삭제 안 했습니다. |
| Ctrl+끌기 | 결과 | `DROPEFFECT_COPY` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | 삭제 안 했습니다. |
| Ctrl+Shift+끌기 | 작업 | 링크 | 링크 |
| Ctrl+Shift+끌기 | 대상 | 원래 항목에 대한 참조 추가 | 원래 항목에 대한 참조 추가 |
| Ctrl+Shift+끌기 | 원본 | 원래 항목에 대한 참조를 유지합니다. | 원래 항목을 유지합니다. |
| Ctrl+Shift+끌기 | 결과 | `DROPEFFECT_LINK` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | `DROPEFFECT_LINK` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. |
| Ctrl+Shift+끌기 | 참고 | Windows 탐색기 바로 가기의 끌어서 놓기 동작과 같습니다. ||
| 잘라내기/붙여넣기 | 작업 | 이동 | 링크 |
| 잘라내기/붙여넣기 | 대상 | 원래 항목에 대한 참조 추가 | 원래 항목에 대한 참조 추가 |
| 잘라내기/붙여넣기 | 원본 | 원래 항목에 대한 참조를 유지합니다.|원래 항목을 유지합니다. |
| 잘라내기/붙여넣기 | 결과 | 항목이 스토리지의 원래 위치에 유지됩니다. | 항목이 스토리지의 원래 위치에 유지됩니다. |
| 복사/붙여넣기 | 작업 | 복사 | 링크 |
...
| Shift + 끌기 | 결과 | `DROPEFFECT_MOVE` 는에서 작업으로 반환 되 `::Drop` 고 항목은 저장소의 원래 위치에 유지 됩니다. | `DROPEFFECT_MOVE` 는에서 작업으로 반환 되 `::Drop` 고 항목은 저장소의 원래 위치에 유지 됩니다. |
| Ctrl + 끌기 | 작업 | 복사 | 복사 |
| Ctrl + 끌기 | 대상 | 대상 위치에 항목을 복사 합니다. | 대상 위치에 항목을 복사 합니다. |
| Ctrl + 끌기 | 원본 | 원본 항목에 대 한 참조를 유지 합니다. | 원본 항목에 대 한 참조를 유지 합니다. |
| Ctrl + 끌기 | 결과 | `DROPEFFECT_COPY` 는에서 작업으로 반환 되 `::Drop` 고 항목은 저장소의 원래 위치에 유지 됩니다. | `DROPEFFECT_COPY` 는에서 작업으로 반환 되 `::Drop` 고 항목은 저장소의 원래 위치에 유지 됩니다. |
...
다음 표에는 소스 항목의 특성 및 혼합 대상 프로젝트에 대해 누른 한정자 키에 따라 수행해야 하는 끌어서 놓기(잘라내기/복사/붙여넣기) 작업이 요약되어 있습니다.

| 한정자 | 범주 | 원본 항목: 참조/링크 | 원본 항목: 물리적 항목 또는 파일 시스템( `CF_HDROP` ) |
| --- | --- | --- | --- |
| 한정자 없음 | 작업 | 이동 | 이동 |
| 한정자 없음 | 대상 | 원래 항목에 대한 참조 추가 | 대상 위치에 항목 복사 |
| 한정자 없음 | 원본 | 원래 항목에 대한 참조를 삭제합니다. | 원래 항목에 대한 참조를 삭제합니다. |
| 한정자 없음 | 결과 | `DROPEFFECT_ MOVE` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | `DROPEFFECT_ MOVE` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에서 삭제됩니다. |
| Shift+끌기 | 작업 | 이동 | 이동 |
| Shift+끌기 | 대상 | 원래 항목에 대한 참조 추가 | 대상 위치에 항목 복사 |
| Shift+끌기 | 원본 | 원래 항목에 대한 참조를 삭제합니다. | 원래 위치에서 항목을 삭제합니다. |
| Shift+끌기 | 결과 | `DROPEFFECT_ MOVE` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | `DROPEFFECT_ MOVE` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에서 삭제됩니다. |
| Ctrl+끌기 | 작업 | 복사 | 복사 |
| Ctrl+끌기 | 대상 | 원래 항목에 대한 참조 추가 | 대상 위치에 항목 복사 |
| Ctrl+끌기 | 원본 | 원래 항목에 대한 참조를 유지합니다. | 원래 항목을 유지합니다. |
| Ctrl+끌기 | 결과 | `DROPEFFECT_ COPY` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | `DROPEFFECT_ COPY` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. |
| Ctrl+Shift+끌기 | 작업 | 링크 | 링크 |
| Ctrl+Shift+끌기 | 대상 | 원래 항목에 대한 참조 추가 | 원본 원본 항목에 대한 참조 추가 |
| Ctrl+Shift+끌기 | 원본 | 원래 항목에 대한 참조를 유지합니다. | 원래 항목을 유지합니다. |
| Ctrl+Shift+끌기 | 결과 | `DROPEFFECT_ LINK` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. | `DROPEFFECT_ LINK` 는 에서 작업으로 `::Drop` 반환되고 항목은 스토리지의 원래 위치에 유지됩니다. |
| 잘라내기/붙여넣기 | 작업 | 이동 | 이동 |
| 잘라내기/붙여넣기 | 대상 | 대상 위치에 항목 복사 | 대상 위치에 항목 복사 |
| 잘라내기/붙여넣기 | 원본 | 원래 항목에 대한 참조를 삭제합니다. | 원래 위치에서 항목을 삭제합니다. |
| 잘라내기/붙여넣기 | 결과 | 항목이 스토리지의 원래 위치에 유지됩니다. | 항목이 스토리지의 원래 위치에서 삭제됩니다. |
| 복사/붙여넣기 | 작업 | 복사 | 복사 |
| 복사/붙여넣기 | 대상 | 원래 항목에 대한 참조 추가 | 대상 위치에 항목 복사 |
| 복사/붙여넣기 | 원본 | 원래 항목을 유지합니다. | 원래 항목을 유지합니다. |
| 복사/붙여넣기 | 결과 | 항목이 스토리지의 원래 위치에 유지됩니다. | 항목은 저장소의 원래 위치에 유지 됩니다. |
