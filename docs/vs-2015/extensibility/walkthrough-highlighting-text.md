...
5. 에서 상속 되는 두 번째 클래스를 만들고 <xref:Microsoft.VisualStudio.Text.Classification.MarkerFormatDefinition> 이름을 HighlightWordFormatDefinition로 다시 만듭니다. 태그에이 형식 정의를 사용 하려면 다음 특성을 사용 하 여 해당 정의를 내보내야 합니다.  
...
7. HighlightWordTag에 대 한 생성자에서 방금 만든 형식 정의의 이름을 전달 합니다.  
...
5. 또한 update 메서드에서 호출 되는 이벤트를 추가 해야 합니다 `TagsChanged` .  
