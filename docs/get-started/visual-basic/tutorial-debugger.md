...
description: Visual Studio 디버거의 기능과 디버거를 시작하고 코드를 단계별로 실행하고 Visual Basic 애플리케이션에서 데이터를 검사하는 방법을 알아봅니다.
...
데모 앱은 Visual Basic이지만, 대부분의 기능은 C#, C++, F#, Python, JavaScript 및 Visual Studio에서 지원하는 다른 언어에 적용할 수 있습니다(F#은 Edit-and-continue를 지원하지 않습니다. F# 및 JavaScript는 **Autos** 창을 지원하지 않습니다). 스크린샷은 Visual Basic에 있습니다.
...
3. **새 프로젝트** 대화 상자의 왼쪽 창에서 **Visual Basic** 을 확장한 후 **.NET Core** 를 선택합니다. 가운데 창에서 **콘솔 앱(.NET Core)** 을 선택합니다. 그런 다음 프로젝트 이름을 *get-started-debugging* 으로 지정합니다.
...
1. **새 프로젝트 만들기** 창에서 검색 상자에 *콘솔* 을 입력합니다. 그런 다음, 언어 목록에서 **Visual Basic** 을 선택한 다음, 플랫폼 목록에서 **Windows** 를 선택합니다. 
   ...
   ![콘솔 앱에 대한 Visual Basic 템플릿을 선택합니다.](../visual-basic/media/vs-2019/get-started-create-console-project.png)
...
1. **새 프로젝트 구성** 창에서 **프로젝트 이름** 상자에 *get-started-debugging* 을 입력합니다. 그리고 **다음** 을 선택합니다.
...
1. *Program.vb* 에서 모든 기본 코드를 다음 코드로 바꿉니다.
...
2. **F5** 또는 **디버깅 시작** 단추 ![디버깅 시작](../../debugger/media/dbg-tour-start-debugging.png "디버깅 시작")을 누릅니다. 그러면 앱이 시작되고, 중단점이 설정된 코드 줄에서 디버거가 실행됩니다.
    ...
    ![중단점 설정 및 적중](../visual-basic/media/get-started-hit-breakpoint-vb.png)
...
1. **F11** 키를 한 번 더 눌러 `SendMessage` 메서드를 한 단계씩 코드 실행합니다.
     ...
     ![F11을 사용하여 한 단계씩 코드 실행](../visual-basic/media/get-started-f11-vb.png "F10 한 단계씩 코드 실행")
...
1. `name += letters[i]` 문에서 일시 중지된 동안 `letters` 변수 위로 마우스를 가져가면 기본값인 배열의 첫 번째 요소 값 `"f"c`이 표시됩니다.
...
1. `letters` 변수를 확장하여 변수가 포함한 요소를 표시합니다.
     ...
     ![지역 창에서 변수 조사](../visual-basic/media/get-started-locals-window-vb.png "지역 창")
