...
description: 워크플로 디자이너에서 기본 Visual Studio 디버거와 유사한 프로세스를 사용 하 여 워크플로와 사용자 지정 작업을 디버깅 하는 기능을 제공 하는 방법을 알아봅니다.
...
- **프로시저 아웃:** **Shift** F11 키를 눌러 작업을 실행 + 합니다. 활동에서 나가기를 수행하면 현재 활동 및 모든 형제 활동이 완료 시까지 실행됩니다. 그런 다음 디버거는 현재 활동의 부모에서 중단합니다. 코드 처리기에서 나갈 때 디버거는 처리기가 연결된 활동에서 중단합니다.
