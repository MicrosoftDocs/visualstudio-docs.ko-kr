...
- 컨트롤러 공급자 인터페이스
  ...
  소스 및 컨트롤러 공급자는 MEF (Managed Extensibility Framework) 구성 요소 부분으로, 소스 및 컨트롤러 클래스를 내보내고, 도구 설명 텍스트 버퍼를 만드는 <xref:Microsoft.VisualStudio.Text.ITextBufferFactoryService>와 같은 서비스 및 브로커를 가져오고, QuickInfo 세션을 트리거하는 <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoBroker>를 담당 합니다.
...
1. VSIX 프로젝트 C# 를 만듭니다. ( **새 프로젝트** 대화 상자에서 **비주얼 C# /확장성**, **VSIX 프로젝트**를 차례로 선택 합니다.) 솔루션 이름을 `QuickInfoTest`로 합니다.
...
4. <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSource>를 구현 하는 클래스를 선언 하 고 이름을 `TestQuickInfoSource`으로 만듭니다.

     [!code-vb[VSSDKQuickInfoTest#2](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-quickinfo-tooltips_2.vb)]
     [!code-csharp[VSSDKQuickInfoTest#2](../extensibility/codesnippet/CSharp/walkthrough-displaying-quickinfo-tooltips_2.cs)]

5. QuickInfo 원본 공급자, 텍스트 버퍼 및 메서드 이름 및 메서드 시그니처의 집합에 대 한 필드를 추가 합니다. 이 예제에서는 메서드 이름 및 시그니처가 `TestQuickInfoSource` 생성자에서 초기화 됩니다.
...
8. <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSource>에서 <xref:System.IDisposable>를 구현 하므로 Dispose () 메서드도 구현 해야 합니다.
...
1. <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSourceProvider>를 구현 하는 `TestQuickInfoSourceProvider` 라는 QuickInfo 원본 공급자를 선언 하 고, "ToolTip QuickInfo Source"의 <xref:Microsoft.VisualStudio.Utilities.NameAttribute> <xref:Microsoft.VisualStudio.Utilities.OrderAttribute>, Before = "default" 및 "text"의 <xref:Microsoft.VisualStudio.Utilities.ContentTypeAttribute>로 내보냅니다.

     [!code-vb[VSSDKQuickInfoTest#7](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-quickinfo-tooltips_7.vb)]
     [!code-csharp[VSSDKQuickInfoTest#7](../extensibility/codesnippet/CSharp/walkthrough-displaying-quickinfo-tooltips_7.cs)]

2. 두 편집기 서비스, <xref:Microsoft.VisualStudio.Text.Operations.ITextStructureNavigatorSelectorService> 및 <xref:Microsoft.VisualStudio.Text.ITextBufferFactoryService>를 `TestQuickInfoSourceProvider`의 속성으로 가져옵니다.

     [!code-vb[VSSDKQuickInfoTest#8](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-quickinfo-tooltips_8.vb)]
     [!code-csharp[VSSDKQuickInfoTest#8](../extensibility/codesnippet/CSharp/walkthrough-displaying-quickinfo-tooltips_8.cs)]

3. <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSourceProvider.TryCreateQuickInfoSource%2A>를 구현 하 여 새 `TestQuickInfoSource`을 반환 합니다.
...
1. <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController>를 구현 하는 클래스를 선언 하 고 이름을 `TestQuickInfoController`으로 만듭니다.
...
5. 컨트롤러가 텍스트 뷰에서 분리 될 때 마우스 호버 이벤트 처리기를 제거 하도록 <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController.Detach%2A> 메서드를 구현 합니다.

     [!code-vb[VSSDKQuickInfoTest#14](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-quickinfo-tooltips_14.vb)]
     [!code-csharp[VSSDKQuickInfoTest#14](../extensibility/codesnippet/CSharp/walkthrough-displaying-quickinfo-tooltips_14.cs)]

6. <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController.ConnectSubjectBuffer%2A> 메서드와 <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController.DisconnectSubjectBuffer%2A> 메서드를이 예제에 대 한 빈 메서드로 구현 합니다.
...
1. <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseControllerProvider>를 구현 하는 `TestQuickInfoControllerProvider` 이라는 클래스를 선언 하 고 "ToolTip QuickInfo Controller"의 <xref:Microsoft.VisualStudio.Utilities.NameAttribute>와 "text"의 <xref:Microsoft.VisualStudio.Utilities.ContentTypeAttribute>로 내보냅니다.
...
3. QuickInfo 컨트롤러를 인스턴스화하여 <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseControllerProvider.TryCreateIntellisenseController%2A> 메서드를 구현 합니다.
...
4. "추가" 항목 중 하나 위로 포인터를 이동 합니다. `add` 메서드의 시그니처와 설명이 표시 됩니다.
