...
description: MSBuild CreateCSharpManifestResourceName 작업을 사용하여 지정된 .resx 파일 이름 또는 다른 리소스에서 C# 스타일 매니페스트 이름을 만듭니다.
...
## <a name="remarks"></a>설명
 ...
 [CreateCSharpManifestResourceName 작업](../msbuild/createcsharpmanifestresourcename-task.md)은 지정된 *.resx* 또는 기타 리소스 파일에 할당할 적절한 매니페스트 리소스 이름을 결정합니다. 이 작업은 리소스 파일에 대한 논리적 이름을 제공한 후 출력 매개 변수에 메타데이터로 추가합니다.
