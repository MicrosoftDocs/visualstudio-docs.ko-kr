...
|<xref:System.Runtime.InteropServices.SafeHandle.DangerousGetHandle%2A?displayProperty=fullName>|Safehandle.dangerousgethandle 메서드는 유효 하지 않은 핸들을 반환할 수 있기 때문에 보안 위험을 초래 합니다. Safehandle.dangerousgethandle 메서드 <xref:System.Runtime.InteropServices.SafeHandle.DangerousAddRef%2A> 를 안전 <xref:System.Runtime.InteropServices.SafeHandle.DangerousRelease%2A> 하 게 사용 하는 방법에 대 한 자세한 내용은 및 메서드를 참조 하세요.|
