...
3. **메시지 표시 안 함**을 선택한 후 **소스**를 선택합니다.
   ...
   메시지를 표시 하지 않으면 `#pragma(warning:` *WarningId* `)` 코드 줄에 대 한 경고를 표시 하지 않는 않는 warningid가 삽입 됩니다.
...
## <a name="c-code-analysis-warnings"></a><a name="Warnings"></a> C + + 코드 분석 경고
