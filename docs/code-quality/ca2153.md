...
CSE는 프로세스의 상태가 손상되었으며 시스템에 의해 catch되지 않았음을 나타냅니다. 손상 된 상태 시나리오에서 일반 처리기는 메서드를 특성으로 표시 하는 경우에만 예외를 catch <xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute?displayProperty=fullName> 합니다. 기본적으로 [CLR (공용 언어 런타임)](/dotnet/standard/clr) 은 cses에 대 한 catch 처리기를 호출 하지 않습니다.
