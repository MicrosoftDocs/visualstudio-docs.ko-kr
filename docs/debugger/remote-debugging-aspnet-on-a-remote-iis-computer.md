...
7. **연결** 에서 **애플리케이션 풀** 을 선택합니다. **DefaultAppPool** 을 열고 애플리케이션 풀 필드를 **관리 코드 없음** 으로 설정합니다.

8. IIS 관리자에서 새 사이트를 마우스 오른쪽 단추로 클릭하고 **사용 권한 편집** 을 선택하고 IUSR, IIS_IUSRS 또는 웹앱에 액세스하도록 구성된 사용자가 읽기 및 실행 권한이 있는 사용자인지 확인합니다.
