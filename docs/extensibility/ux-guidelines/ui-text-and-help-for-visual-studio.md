...
- **컴퓨터별.** 모호한 표현을 방지 하 고 관련 개체의 특정 이름 및 위치를 제공 합니다. 예를 들어 "문자가 잘못되었습니다"와 같은 오류 메시지는 유용하지 않습니다. 어떤 문자인가요? "파일을 찾을 수 없습니다." 어떤 파일인가요?

- **정중.** 사용자를 요망하거나 좋도록 하지 마세요. 악의적이거나 공격적인 언어(종료, 실행, 종료, 치명적, 불법)를 방지합니다. 대문자로 표시되고 읽을 수 없는 대문자 텍스트는 사용하지 마십시오. 을 사용하지 마세요.

- **정답입니다.** 올바른 맞춤법 및 문법(알파에서도)을 사용합니다. 오타가 전문적이지 않고 적합하지 않습니다.

- **상황에 따라 적절합니다.** 적절한 단추 텍스트를 사용합니다. "확인" 단추를 사용하지 않고 대신 "계속" 또는 "예/아니요"를 사용합니다.

### <a name="error-message-examples"></a>오류 메시지 예제

|좋음|나쁨|
|----------|---------|
|"전화 걸기한 번호가 더 이상 서비스되지 않습니다. 번호를 확인하고 다시 전화를 걸거나 연산자의 경우 0으로 전화를 걸세요."|- "오류(449): 잘못된 숫자"<br />- "이 처리되지 않은 예외 오류는 작업이 성공적으로 완료되었음을 나타냅니다."<br /><br /> ![Visual Studio의 잘못된 오류 메시지](../../extensibility/ux-guidelines/media/0602-a_errordialog.png "0602-a_ErrorDialog")|

## <a name="accessing-help"></a>도움말 액세스

### <a name="overview"></a>개요
 MSDN의 설명서 외에도 Visual Studio 사용자에게는 UI에 있는 동안 사용자를 지원하기 위한 여러 액세스 지점이 있습니다. 이러한 액세스 지점을 일관되게 사용할 수 있도록 하려면 기능 팀은 환경에서 제공하는 도움말 시스템을 활용해야 합니다. 이러한 액세스 지점은 다음과 같습니다.

- **대화 상자의 지침 및 추가 텍스트입니다.** UI 표면에서 또는 InfoTip 아이콘 위로 마우스를 가져가면 사용할 수 있는 방향 또는 설명을 제공하는 정적 텍스트입니다.

- **F1** 도움말(편집기만 해당). Visual Studio 편집기 내에서 사용자는 언제든지 F1 키를 누르면 현재 선택 항목과 관련된 도움말 항목이 표시됩니다. F1과 연결된 항목이 적절하고 정보적인지 확인합니다.

- **도움말 항목에 대한 하이퍼링크.** 사용자가 기술, 기능 또는 작업을 수행하는 방법에 대한 정보를 자세히 학습하는 데 도움이 되는 항목을 시작하는 대화 상자, 도구 창 또는 디자인 표면 내의 하이퍼링크입니다.

- **스마트 태그 및 빌드 대화 상자와 같은 도우미 UI 메커니즘** 이러한 메커니즘은 사용자가 UI 요소를 이해하는 데 도움이 되거나 스마트 태그 또는 작성기 대화 상자와 같은 작업을 용이하게 합니다.

- **UI 도움말 단추(사용되지** 않습니다.) 관련 F1 도움말 항목에 대한 액세스 권한을 제공하는 제목 표시줄에 표시되는 표시기입니다.

### <a name="text"></a>텍스트

#### <a name="instructional-and-supplemental-text-in-dialogs"></a>대화 상자의 지침 및 추가 텍스트
 복잡한 작업을 지원하는 대화 상자에서는 UI 내에서 종종 대화 상자의 맨 위에 또는 복잡한 컨트롤에 가까운 지침 텍스트를 제공해야 할 수 있습니다. 작성 스타일에 대한 자세한 내용은 [UI 텍스트 및 용어를](../../extensibility/ux-guidelines/ui-text-and-help-for-visual-studio.md#BKMK_UITextAndTerminology) 참조하세요.

#### <a name="infotips"></a>InfoTips
 종종 지침 텍스트가 너무 길어 UI에 배치할 수 없거나 숙련된 사용자에게 복잡해 보이는 새 사용자에게만 유용할 수 있습니다. 이 경우 지침/정보 텍스트를 InfoTip 아래에 도구 설명으로 배치해야 합니다.

 InfoTips는 관련된 컨트롤 근처에 배치해야 하며, 특정 InfoTip 아이콘을 사용해야 합니다. 이 아이콘은 눈에 띄지 않습니다.

 ![Visual Studio의 정보 팁](../../extensibility/ux-guidelines/media/0601-d_infotip.png "0601-d_InfoTip")

 **Visual Studio InfoTip의 예**
...
#### <a name="hyperlinks-to-help-topics"></a>도움말 항목에 대한 하이퍼링크
 하이퍼링크를 사용하여 작업을 수행하거나, IDE 내에서 탐색하거나, 브라우저에서 도움말을 시작할 수 있습니다. 시각적 개체 및 레이아웃 지침에 대한 언어 및 07.10.01 단추 및 하이퍼링크에 대한 자세한 내용은 [UI 텍스트](../../extensibility/ux-guidelines/ui-text-and-help-for-visual-studio.md#BKMK_UITextAndTerminology) 및 용어를 참조하세요.

#### <a name="help--buttons-in-dialog-title-bars-deprecated"></a>대화 상자 제목 표시줄의 도움말 [?] 단추(사용되지 않음)
 대부분의 경우 대화 상자의 제목 표시줄에 있는 도움말 [?] 단추는 더 이상 사용되지 않습니다. UI 항목은 더 이상 doc 모델의 일부가 아니므로 연결할 관련 항목이 없을 수 있습니다. 기본적으로 제목 표시줄 단추는 F1 도움말과 동일하며 대화 상자에서 더 이상 필요하지 않습니다. 일부 경우에는 하이퍼링크가 최신 UI에서 더 일반적으로 사용되지만 더 많은 개념적 또는 절차적 정보를 사용할 수 있음을 나타내는 표시로 계속 사용할 수 있습니다.

##### <a name="dialogs-created-through-the-environment"></a>환경을 통해 만든 대화 상자
 많은 셸 대화 상자는 **VBDialogBoxParam** 함수를 통해 만들어집니다. 이 공유 함수는 대화 상자에서 **?**  이전 버전과 호환되고 연장 가능한 아키텍처를 유지하는 동안 단추가 표시됩니다.

 특히 **VBDialogBoxParam** 함수는 ID가 **IDHELP(9)** 또는 레이블이 도움말 또는&**도움말인** 단추에 대한 대화 상자 템플릿을 확인합니다.  도움말 단추가 있으면 숨겨지고 **WS_EX_CONTEXTHELP** 스타일이 대화 상자에 추가되어 **?** 대화 상자의 제목 표시줄에 있는 단추입니다.

 대화 상자가 만들어지면 dialog proc를 스택으로 푸시하고 **DialogPreProc** 라는 전처리 대화 상자를 통해 대화 상자를 호출합니다. 때는 **?** 단추를 클릭하면 **SC_CONTEXTHELP** **WM_SYSCOMMAND** 대화 상자로 전송됩니다. **DialogPreProc는** 이 명령을 캡처하여 원래 대화 상자에 전달되는 **WM_HELP** 메시지로 변경합니다.

 환경에서 만든 대부분의 대화 상자에는 도움말 단추가 있습니다. 대화 상자가 표시되면 도움말 단추가 자동으로 숨겨지고 **?** 단추가 작동합니다. **?** 단추는 Windows에서 제거되거나 변경됩니다. 이 솔루션을 사용하면 원래 도움말 단추로 신속하게 다시 이동할 수 있습니다.

 이 솔루션은 버그를 일으킬 수 있는 4가지 가정을 만듭니다.

- 대화 상자의 도움말  단추는 IDHELP(9)입니다.

- 도움말 단추를 숨기면 대화 상자가 올바르게 표시됩니다.

- 대화 상자는 winproc를 대체하지 않습니다.

- 대화 상자는 다른 대화 상자 내에 포함되지 않습니다.

  대화 상자가 msenv 내에 있고 **VBDialogBoxParam을** 사용하지 않는 경우 고유한 처리기를 구현하기 전에 **VBDialogBoxParam** 활용을 조사합니다.

##### <a name="dialogs-created-through-other-packages"></a>다른 패키지를 통해 만든 대화 상자
 msenv 외부에 있는 대화에 대한 고유한 솔루션을 구현할 수 있습니다. VSPackage의 공유 대화 클래스의 경우 단추를 제목 표시줄로 이동하거나 각 대화 상자에서 처리기를 구현하는 것이 좋습니다. 다음 코드는 시작하는 데 도움이 되는 구현의 기본 구조입니다.
...
##### <a name="help-buttons-in-managed-code"></a>관리 코드의 도움말 단추
 관리 코드에서는 창 제목 표시줄 도움말 단추의 기본 동작을 쉽게 재정의할 수 있습니다. 다음은 이 동작을 보여 주는 완전한 데모 애플리케이션입니다. 기본적으로 폼의 **WndProc** 메서드를 재정의한 다음 **SC_CONTEXTHELP** 메시지가 가로채면 F1 도움말 요청을 발생시켜야 합니다.
