---
title: 비주얼 스튜디오용 글꼴 및 서식 지정 | 마이크로 소프트 문서
ms.date: 04/26/2017
ms.topic: conceptual
...
- **환경 글꼴** - 대화 상자, 메뉴, 도구 창 및 문서 창을 포함한 모든 인터페이스 요소에 사용되는 IDE(통합 개발 환경)의 기본 글꼴입니다. 기본적으로 환경 글꼴은 현재 버전의 Windows에서 9pt Segoe UI로 표시되는 시스템 글꼴에 연결됩니다. 모든 인터페이스 요소에 하나의 글꼴을 사용하면 IDE 전체에서 일관된 글꼴 모양을 유지할 수 있습니다.
...
2. 인터페이스의 다른 모든 요소는 환경 글꼴 설정에 연결되어야 하며 환경의 전역 변경 사항에 응답해야 합니다. 여기에는 다음이 포함됩니다(이에 국한되지 않음).
    ...
    - 전구 메뉴 텍스트, 빠른 찾기 편집기 창 및 창으로 이동과 같은 편집기 장식의 텍스트

    - **파일에서 찾기** 또는 **리팩터링과** 같은 대화 상자의 텍스트 레이블 지정

### <a name="accessing-the-environment-font"></a>환경 글꼴 에 액세스
 네이티브 또는 WinForms 코드에서 환경에서 인터페이스를 쿼리한 `IUIHostLocale::GetDialogFont` 후 메서드를 호출하여 `SID_SUIHostLocale` 환경 글꼴에 액세스할 수 있습니다.

 WPF(Windows 프레젠테이션 파운데이션)의 경우 WPF `DialogWindow` 클래스 대신 셸 클래스에서 대화 창 클래스를 `Window` 파생시됩니다.
...
```
 ...
 대화 상자를 표시하려면 클래스`ShowModal()`에서 "를 `ShowDialog()`호출합니다. `ShowModal()`셸에서 올바른 모달 상태를 설정하고 대화 상자가 부모 창의 가운데에 있는지 확인하는 등의 것입니다.
...
```
 ...
 `ShowModal`밥을 반환? (nullable Boolean)와 `DialogResult`에서 필요한 경우 사용할 수 있습니다. 대화 상자가 **확인으로**닫힌 경우 반환 값은 true입니다.

 대화 상자가 아니며 팝업 창또는 Win32/WinForms 부모 `HwndSource`창의 WPF 자식 창과 같이 자체적으로 호스팅되는 일부 WPF UI를 표시해야 `FontFamily` 하는 `FontSize` 경우 WPF 요소의 루트 요소와 를 설정해야 합니다. 셸은 기본 창에서 속성을 설정하지만 a를 `HWND`지나상속되지는 않습니다. 셸은 다음과 같이 속성을 바인딩할 수 있는 리소스를 제공합니다.
...
### <a name="formatting-scalingbolding-reference"></a><a name="BKMK_Formatting"></a>서식 지정(크기 조정/굵게) 참조
 일부 대화 상자에서는 특정 텍스트가 굵게 또는 환경 글꼴 이외의 크기여야 합니다. 이전에는 환경 글꼴보다 큰 글꼴이`environment font +2`"또는 이와 유사한 글꼴로 코딩되었습니다. 제공된 코드 조각을 사용하면 높은 DPI 모니터를 지원하고 표시 텍스트가 항상 올바른 크기와 무게(예: Light 또는 Semilight)로 표시되도록 합니다.

> [!NOTE]
> 서식을 적용하기 전에 텍스트 스타일에 있는 지침을 따르고 있는지 [확인합니다.**](../../extensibility/ux-guidelines/fonts-and-formatting-for-visual-studio.md#BKMK_TextStyle)

 환경 글꼴의 크기를 조정하려면 TextBlock 또는 레이블의 스타일을 표시된 대로 설정합니다. 제대로 사용된 이러한 각 코드 조각은 적절한 크기와 무게 변형을 포함하여 올바른 글꼴을 생성합니다.

 여기서`vsui`" " 네임스페이스에 `Microsoft.VisualStudio.Shell`대한 참조입니다.
...
**에 대 한 사용:** (드문) 고유 한 브랜드 UI, 시작 페이지에서 처럼

::: moniker-end

::: moniker range=">=vs-2019"

**에 대 한 사용:** (드문) 고유 한 브랜드 UI

::: moniker-end

**절차 코드:** 이전에 `textBlock` 정의된 TextBlock이며 `label` 이전에 정의된 레이블은 다음과 같은 위치입니다.
 ...
 **로 나타납니다:** 28 pt Segoe UI 라이트 **사용:** 큰 서명 대화 상자 제목, 보고서의 주요 제목
 ...
 **다음과 같이 나타납니다 :** 18 pt Segoe UI 세미 라이트 **사용 :** 부제목, 중소 대화 상자의 제목
 ...
 **다음과 같이 나타납니다 :** 14 pt Segoe UI **사용 :** 문서 잘 UI 또는 보고서의 섹션 제목
 ...
 **다음과 같이 나타납니다 :** 12 pt Segoe UI **사용 :** 서명 대화 상자의 작은 부제목 및 문서 잘 UI
 ...
 **다음과 같이 나타납니다:** 11 pt Segoe UI **에 대 한 사용:** 서명 대화 상자의 섹션 제목, 트리 보기의 상단 노드, 세로 탭 탐색
 ...
 **다음과 같이 나타납니다:** 굵게 표시된 9 pt Segoe UI **사용:** 서명 대화 상자, 보고서 및 문서 웰 UI의 레이블 및 하위 헤드
...
### <a name="localizable-styles"></a>지역화 가능한 스타일
 ...
 해결 방법은 대화 상자 양식의 `FontChanged` 이벤트를 연결하는 것입니다. 이 `FontChanged` 경우 모든 컨트롤을 걷고 글꼴이 설정되어 있는지 확인합니다. 설정된 경우 양식의 글꼴과 컨트롤의 이전 글꼴 스타일에 따라 새 글꼴로 변경합니다. 코드에서 이것의 예는 다음과 같은 것입니다.
...
```
 ...
 이 코드를 사용하면 폼의 글꼴이 업데이트될 때 컨트롤 글꼴도 업데이트됩니다. 대화 상자가 인스턴스를 `IUIService` 얻지 못하고 `FontChanged` 이벤트가 발생하지 않을 수 있으므로 폼의 생성자에서 이 메서드를 호출해야 합니다. 후킹을 `FontChanged` 사용하면 대화 상자가 이미 열려 있는 경우에도 대화 상자가 새 글꼴을 동적으로 선택할 수 있습니다.

### <a name="testing-the-environment-font"></a>환경 글꼴 테스트
 UI가 환경 글꼴을 사용하고 크기 설정을 준수하도록 하려면 **환경 > 환경 > 옵션 > > 도구를** 열고 "환경 글꼴 표시:" 드롭다운 메뉴에서 "환경 글꼴"을 선택합니다.

 ![도구 &gt; 옵션 대화 상자의 글꼴 및 색상 설정](../../extensibility/ux-guidelines/media/0201-a_optionsfonts.png "0201-a_OptionsFonts")<br />도구 &gt; 옵션 대화 상자의 글꼴 및 색상 설정
 ...
 ![환경 글꼴을 준수하지 않는 UI 텍스트의 예](../../extensibility/ux-guidelines/media/0201-b_wrongfontdialog.png "0201-b_WrongFontDialog")<br />환경 글꼴을 준수하지 않는 UI 텍스트의 예
...
- 머리글에서 색상을 사용하는 경우 명암비 및 기타 접근성 고려 사항을 포함하여 [Windows 색상 지침을](/windows/desktop/uxguide/vis-color)준수해야 합니다.
...
|||
|-|-|
|**사용 법:** 드문. 고유한 브랜드 UI만.<br /><br /> **다음을 수행합니다.**<br /><br /> - 사용 문장 케이스<br />- 항상 가벼운 무게를 사용<br /><br /> **안 함:**<br /><br /> - 시작 페이지와 같은 서명 UI 이외의 UI에 사용<br />- 굵게, 기울임꼴, 또는 굵은 기울임꼴<br />- 본문 텍스트에 사용<br />- 도구 창에서 사용|**로 나타납니다 :** 34 pt 세고에 UI 라이트<br /><br /> **시각적 인 예 :**<br /><br /> *현재 사용되지 않습니다. 비주얼 스튜디오 2017 시작 페이지에서 사용할 수 있습니다.*|
...
|||
|-|-|
|**사용 법:** 기타 모든 텍스트<br /><br /> **다음을 수행합니다.** 사용 문장 케이스<br /><br /> **하지 마십시오:** 기울임꼴 또는 굵은 기울임꼴|**다음과 같이 나타납니다 :** 9 pt 세고에 UI<br /><br /> **시각적 인 예 :**<br /><br /> ![환경 글꼴의 예](../../extensibility/ux-guidelines/media/0202-g_ef.png "g_EF 0202-")|
