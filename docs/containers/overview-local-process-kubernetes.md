...
> [!NOTE]
> 클러스터에 연결하는 시간(및 추가로 15분) 동안 Kubernetes를 사용한 로컬 프로세스는 로컬 컴퓨터에 대한 관리자 권한이 있는 *EndpointManager*라는 프로세스를 실행합니다.
...
기본적으로 Kubernetes와 함께 로컬 프로세스를 사용하는 경우 서비스의 모든 트래픽이 개발 컴퓨터로 리디렉션됩니다. 하위 도메인에서 시작되는 서비스에 대한 요청만 개발 컴퓨터로 리디렉션하는 라우팅 기능을 사용할 수도 있습니다. 해당 라우팅 기능을 사용하면 Kubernetes와 함께 로컬 프로세스를 사용하여 격리 상태로 개발하고 클러스터의 다른 트래픽이 중단되지 않도록 할 수 있습니다.

다음 애니메이션에서는 동일한 클러스터에서 두 명의 개발자가 격리 상태로 작업하는 상황을 보여 줍니다.

![격리를 보여 주는 애니메이션 GIF](media/local-process-kubernetes/lpk-graphic-isolated.gif)

격리 상태로 작업하도록 설정하면 Kubernetes를 사용한 로컬 프로세스가 Kubernetes 클러스터에 연결하는 것 외에도 다음과 같은 작업을 수행합니다.
...
Kubernetes를 사용한 로컬 프로세스가 Kubernetes 클러스터에 Azure Dev Spaces가 사용하도록 설정된 것을 감지하는 경우, Kubernetes와 함께 로컬 프로세스를 사용하려면 Azure Dev Spaces를 사용하지 않도록 설정하라는 메시지가 표시됩니다.
...
클러스터와 연결을 끊으면 기본적으로 Kubernetes를 사용한 로컬 프로세스가 모든 envoy pod 및 중복 서비스를 제거합니다. 

> [참고] 라우팅 관리자 배포 및 서비스는 네임스페이스에서 계속 실행됩니다. 배포 및 서비스를 제거하려면 네임스페이스에 대해 다음 명령을 실행합니다.
>
> ```azurecli
...
Kubernetes를 사용한 로컬 프로세스를 사용하여 클러스터에 연결하는 경우 클러스터의 진단 로그가 개발 컴퓨터의 *TEMP* 디렉터리에서 *Local Process with Kubernetes* 폴더에 기록됩니다.
...
* Kubernetes를 사용한 로컬 프로세스는 Azure Dev Spaces가 사용하도록 설정된 클러스터에 사용할 수 없습니다.

### <a name="local-process-with-kubernetes-and-clusters-with-azure-dev-spaces-enabled"></a>Kubernetes를 사용한 로컬 프로세스 및 Azure Dev Spaces가 사용 설정된 클러스터

Kubernetes를 사용한 로컬 프로세스는 Azure Dev Spaces가 사용하도록 설정된 클러스터에 사용할 수 없습니다. Azure Dev Spaces가 사용하도록 설정된 클러스터에서 Kubernetes를 사용한 로컬 프로세스를 사용하려면 Azure Dev Spaces를 사용하지 않도록 설정한 후에 클러스터에 연결해야 합니다.
