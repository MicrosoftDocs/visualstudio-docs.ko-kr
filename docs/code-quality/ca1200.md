---
title: 'CA1200: 접두사를 사용하여 cref 태그 사용 방지'
ms.date: 09/16/2019
ms.topic: reference
f1_keywords:
...
XML 문서 태그의 [cref](/dotnet/csharp/programming-guide/xmldoc/cref-attribute) 특성은 "코드 참조"를 의미 합니다. 태그의 내부 텍스트를 형식, 메서드, 속성 등의 코드 요소로 지정합니다. `cref`컴파일러가 참조를 확인 하는 것을 방지 하므로 접두사와 함께 태그를 사용 하지 마십시오. 또한 Visual Studio IDE (통합 개발 환경)에서 리팩터링 중에 이러한 기호 참조를 찾아 업데이트할 수 없습니다. 접두사 없이 전체 구문을 사용 하 여 cref 태그에서 기호 이름을 참조 하는 것이 좋습니다.
...
- [XML 주석을 사용하여 코드 문서화](/dotnet/csharp/codedoc)
