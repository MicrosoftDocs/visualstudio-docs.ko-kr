...
경고 C28251: 함수에 대 한 주석이 일치 하지 않습니다 .이 인스턴스에 오류가 있습니다.  
 ...
 비교는 항상 발견된 첫 번째 선언과 현재 항목 사이에 수행됩니다. 추가 선언이 있으면 각 선언이 두 개 그룹에서 확인됩니다. 두 개 이상의 선언/정의가 있는지 확인할 수 있지만 현재까지는 쌍 이외의 형태로 비교를 수행할 수 없습니다.  위의 *텍스트* 필드에는 두 인스턴스 간에 차이가 있는 주석 목록 (매우 낮은 수준)이 포함 되어 있습니다.  
