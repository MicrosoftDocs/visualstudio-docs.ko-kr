...
## <a name="unmanaged-vspackages"></a>관리 되지 않는 Vspackage  
 관리 되지 않는 VSPackage는 모든 위치에 설치할 수 있는 COM 서버입니다. 해당 등록 정보는 해당 위치를 정확 하 게 반영 해야 합니다. 설치 관리자 UI (사용자 인터페이스)는 기본 위치를 ProgramFilesFolder Windows Installer 속성의 하위 디렉터리로 제공 해야 합니다. 예를 들면 다음과 같습니다.  
...
## <a name="satellite-dlls"></a>위성 DLL  
 ...
 [Vspackage를 관리](../../extensibility/managing-vspackages.md) 하는 경우 레지스트리 항목에서 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 실제로 VSPACKAGE의 위성 DLL을 검색 하는 위치를 제어 함을 나타냅니다. 그러나는 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] LCID 값에 대해 라는 하위 디렉터리에 다음 순서로 위성 DLL을 로드 하려고 시도 합니다.  
...
5. 미국 영어 (.\ 1033 또는 .\0x409).  
   ...
   VSPackage DLL이 리소스를 포함 하 고 SatelliteDll\DllName 레지스트리 항목이이를 가리키는 경우는 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 위의 순서로 로드 하려고 시도 합니다.  
