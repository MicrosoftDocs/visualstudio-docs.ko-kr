...
> [!NOTE] 
> 최상의 환경을 위해 전체 프로세스 동안 브라우저를 열어 두세요. 브라우저를 닫으면 추가 인증 프롬프트가 트리거될 수 있습니다. 
...
**자격 증명 다시 입력**을 클릭하면 시스템의 기본 웹 브라우저가 열리고 자격 증명을 자동으로 새로 고치려고 합니다. 실패하면 계정에 로그인하고 필요한 MFA 정책의 유효성을 검사하라는 메시지가 표시됩니다. 
