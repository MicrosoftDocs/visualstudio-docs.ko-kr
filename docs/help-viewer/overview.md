...
|즐겨찾기|**즐겨찾기에 추가** 단추를 선택 하 여 "즐겨찾기" 항목을 만들 수 있으며,이 탭에 항목이 나타납니다. **기록** 섹션에는 최근에 본 항목의 목록이 표시 됩니다.|
...
**콘텐츠 관리** 탭을 사용 하 여 콘텐츠를 설치, 업데이트, 이동 및 삭제할 수 있습니다. 탭의 위쪽에서 **설치 소스** 컨트롤을 사용 하 여 네트워크 위치에서 책을 설치할지 또는 디스크 또는 URI에서 책을 설치할지를 지정할 수 있습니다. **로컬 저장소 경로** 상자에는 로컬 컴퓨터에서 책이 설치된 위치가 표시되며, **이동** 단추를 선택하여 책을 다른 위치로 이동할 수 있습니다.
