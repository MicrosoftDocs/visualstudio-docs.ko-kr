...
## <a name="signature-of-an-operation"></a>작업 서명
 작업 서명은 UML 클래스 다이어그램의 클래스 또는 인터페이스에서 서명을 나타내는 텍스트 줄입니다. 형태는 다음과 같습니다.

 \+ OperationName (parameter1: Type1 [*], ...): ReturnType [ \* ]

 \+ 공용 표시 여부를 나타냅니다. 기타 허용되는 값은 -(private), #(protected), ~(package)입니다.

 `OperationName` 는 **정적** 속성이 true 이면 밑줄이 표시 되 고, **추상** 속성이 true 이면 기울임꼴로 표시 됩니다.

 `: ReturnType` 반환 형식이 정의 되어 있지 않으면이 생략 됩니다.

 `[*]` 매개 변수 또는 반환 형식의 복합성을 나타냅니다. 복합성이 1이면 생략됩니다.
...
|  **Preconditions**  |    (없음)    |                                                                                                                            작업 실행이 시작되기 전에 시스템 상태에 대한 가정을 지정하는 선택적 조건입니다.                                                                                                                            |
...
|  **다중성**   |      1       |                                 **1** -지정 된 형식의 단일 값입니다.<br /><br /> **0 ..1** `null` -일 수 있습니다.<br /><br /> \* -지정 된 형식의 값 컬렉션입니다.<br /><br /> **1.. \\ ** \* -하나 이상의 값을 포함 하는 컬렉션입니다.<br /><br /> *n* `..` *m* -및 값 사이에를 포함 하는 컬렉션입니다 `n` `m` .                                  |
|   **순서 지정됨**    |    False     |                                                                                                                                             true이면 컬렉션이 순차적 목록을 구성합니다. **복합성이** 1 보다 많은 경우                                                                                                                                              |
