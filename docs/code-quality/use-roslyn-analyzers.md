...
| 기본 | `default` | 규칙의 기본 심각도에 해당 합니다. 규칙의 기본값을 확인 하려면 속성 창를 확인 합니다. | 규칙의 기본 심각도에 해당 합니다. |
...
- Visual Studio의 규칙 집합 편집기 (Visual Studio 2019 16.5 이상 필요). 프로젝트에서 특정 규칙 집합 파일을 이미 사용 하는 경우 `CodeAnalysisRuleSet` Visual Studio 내의 규칙 집합 편집기에서 해당 하는 EditorConfig 파일로 변환할 수 있습니다.
    ...
    3. **저장** 단추를 **클릭** 하 여 editorconfig 파일을 생성 합니다.
...
1. **솔루션 탐색기**에서 활성 규칙 집합 파일을 두 번 클릭 하거나, **참조**분석기 노드의 오른쪽 클릭 메뉴에서 **활성 규칙 집합 열기** 를 선택  >  **Analyzers** 하거나, 프로젝트에 대 한 **코드 분석** 속성 페이지에서 **열기** 를 선택 하 여 활성 규칙 집합 파일을 엽니다.
