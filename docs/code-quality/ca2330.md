---
title: 'CA2330: 역직렬화할 때 JsonSerializer에 보안 구성이 있는지 확인하세요.'
...
이 규칙은 [Newtonsoft.json JsonSerializer](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_JsonSerializer.htm) 인스턴스에 대해 다음 두 가지 조건이 모두 충족 될 때 발생 합니다.
...
이 규칙은 [CA2329](ca2329.md)와 비슷하지만, 직렬 변환기가에서 비보안 방식 구성 되었는지 여부를 분석할 수 없습니다.
...
- [TypeNameHandling](https://www.newtonsoft.com/json/help/html/P_Newtonsoft_Json_JsonSerializerSettings_TypeNameHandling.htm) 속성이이 아닌 값인 경우 [newtonsoft.json](https://www.newtonsoft.com/json/help/html/P_Newtonsoft_Json_JsonSerializer_SerializationBinder.htm) 속성은 항상 설정 되어 `None` 있습니다.
...
| 옵션 값 | 요약 |
| --- | --- |
|`dotnet_code_quality.CA2330.excluded_symbol_names = MyType` | 컴파일에 ' MyType ' 이라는 모든 기호를 찾습니다.
|`dotnet_code_quality.CA2330.excluded_symbol_names = MyType1|MyType2` | 컴파일에 ' MyType1 ' 또는 ' MyType2 ' 라는 모든 기호를 찾습니다.
|`dotnet_code_quality.CA2330.excluded_symbol_names = M:NS.MyType.MyMethod(ParamType)` | 지정 된 정규화 된 시그니처와 ' MyMethod ' 특정 메서드를 일치 시킵니다.
|`dotnet_code_quality.CA2330.excluded_symbol_names = M:NS1.MyType1.MyMethod1(ParamType)|M:NS2.MyType2.MyMethod2(ParamType)` | 특정 메서드 ' MyMethod1 ' 및 ' MyMethod2 '를 해당 하는 정규화 된 시그니처와 일치 시킵니다.
