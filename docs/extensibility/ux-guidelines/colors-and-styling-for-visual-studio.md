---
title: 비주얼 스튜디오를 위한 색상 및 스타일링 | 마이크로 소프트 문서
ms.date: 07/31/2017
ms.topic: conceptual
...
## <a name="use-color-in-visual-studio"></a>비주얼 스튜디오에서 색상 사용
...
- Visual Studio에서 색상을 정의하는 데 사용되는 다양한 서비스와 시스템을 이해합니다.
...
> [!NOTE]
> UI 요소에 육신, RGB 또는 시스템 색상을 하드코딩하지 마십시오. 이 서비스를 사용하면 색조를 유연게 조정할 수 있습니다. 또한 서비스가 없으면 [VSColor 서비스의](../../extensibility/ux-guidelines/colors-and-styling-for-visual-studio.md#BKMK_TheVSColorService)테마 스위칭 기능을 활용할 수 없습니다.
...
| --- | --- | --- |
| 포함된 대화 상자 또는 독립 실행형 대화 상자가 있습니다. | **시스템 색상** | 운영 체제에서 일반적인 대화 상자 컨트롤과 같은 UI 요소의 색상과 모양을 정의할 수 있는 시스템 이름입니다. |
...
| 최종 사용자가 UI 또는 콘텐츠(예: 텍스트 편집기 또는 특수 디자이너 창)를 사용자 지정하도록 허용하려고 합니다. | **최종 사용자 사용자 지정**<br /><br />**(도구 &gt; 옵션 대화 상자)** | **도구 &gt; 옵션** 대화 상자의 "글꼴 및 색상" 페이지에 정의된 설정 또는 하나의 UI 기능에 특정한 특수 페이지입니다. |

### <a name="visual-studio-themes"></a>비주얼 스튜디오 테마

비주얼 스튜디오는 빛, 어두운, 파란색의 세 가지 색상 테마를 갖추고 있습니다. 또한 접근성을 위해 설계된 시스템 전체 색상 테마인 고대비 모드를 감지합니다.

사용자는 Visual Studio를 처음 사용하는 동안 테마를 선택하라는 메시지가 표시되며 **나중에 &gt; &gt; 도구 &gt; 옵션 환경 일반으로** 이동하여 "색상 테마" 드롭다운 메뉴에서 새 테마를 선택하여 테마를 전환할 수 있습니다.
...
경우에 따라 코드 편집기 또는 디자인 표면을 만들 때와 같이 최종 사용자가 UI를 사용자 지정하도록 허용할 수 있습니다. 사용자 지정 가능한 UI 구성 요소는 ** &gt; 도구 옵션** 대화 상자의 **글꼴 및 색상** 섹션에서 찾을 수 있으며, 여기서 사용자는 전경 색상, 배경 색 또는 둘 다를 변경할 수 있습니다.
...
환경 색상 서비스는 UI 구성 요소에 대한 .pkgdef에 정의된 VSColors를 읽습니다. 그런 다음 이러한 VSColors는 XAML 태그 또는 코드에서 `IVsUIShell5.GetThemedColor` 참조되며 `DynamicResource` 또는 매핑을 통해 로드됩니다.
...
셸은 색상에 `COLORREF` 대한 액세스를 제공하는 서비스를 제공합니다. 서비스/인터페이스는 다음과 같은 것입니다.

```
IVsUIShell2::GetVSSysColorEx(VSSYSCOLOR dwSysColIndex, DWORD *pdwRGBval)
```

파일 VSShell80.idl에서 열거형에는 `__VSSYSCOLOREX` 쉘 색상 상수가 있습니다. 이를 사용하려면 MSDN에 `enum __VSSYSCOLOREX` 문서화된 값 중 하나 또는 Windows 시스템 API가 `GetSysColor`허용하는 일반 인덱스 번호 중 하나를 인덱스 값으로 전달합니다. 이렇게 하면 두 번째 매개 변수에 사용해야 하는 색상의 RGB 값이 다시 표시됩니다.

새 색상으로 펜이나 브러시를 저장하는 경우 `AdviseBroadcastMessages` Visual Studio 셸에서 벗어나서 `WM_SYSCOLORCHANGE` 수신 `WM_THEMECHANGED` 및 메시지를 수신해야 합니다.

네이티브 코드의 색상 서비스에 액세스하려면 다음과 유사한 호출을 수행합니다.
...
> [!NOTE]
> 반환되는 `COLORREF` `GetVSSysColorEx()` 값에는 테마 색상의 R, G, B 구성 요소만 포함됩니다. 테마 항목에서 투명도를 사용하는 경우 알파 채널 값은 반환하기 전에 삭제됩니다. 따라서 투명도 채널이 중요한 위치에 환경 색상을 사용해야 하는 경우 이 `IVsUIShell5.GetThemedColor` 항목의 `IVsUIShell2::GetVSSysColorEx`설명과 같이 대신 사용해야 합니다.
...
응용 프로그램의 로 내보낸 값을 통해 Visual Studio `ResourceDictionary`색상에 바인딩할 수 있습니다. 다음은 색상 테이블의 리소스를 사용하고 XAML의 환경 글꼴 데이터에 바인딩하는 예제입니다.
...
관리 되는 코드의 경우 셸의 관리되는 패키지 프레임 워크 라이브러리 ()`Microsoft.VisualStudio.Shell.12.0.dll`테마 색상의 사용을 용이하게 하는 도우미 클래스의 몇 가지 가 포함되어 있습니다.

MPF의 클래스의 `Microsoft.VisualStudio.Shell.VsColors` 도우미 메서드에는 `GetThemedGDIColor()` `GetThemedWPFColor()`및 가 포함됩니다. 이러한 도우미 메서드는 WinForms 또는 WPF `System.Drawing.Color` `System.Windows.Media.Color`UI에서 사용할 테마 항목의 색상 값을 반환합니다.
...
클래스 메서드는 `VsColors` VSColor 서비스를 쿼리하여 호출될 때마다 색상 값을 반환합니다. 색상 값을 얻으려면 `System.Drawing.Color`성능이 더 좋은 대안은 대신 VSColor `Microsoft.VisualStudio.PlatformUI.VSThemeColor` 서비스에서 가져온 색상 값을 캐시하는 클래스의 메서드를 사용하는 것입니다. 클래스는 셸 브로드캐스트 메시지 이벤트에 내부적으로 구독하고 테마 변경 이벤트가 발생하면 캐시된 값을 삭제합니다. 또한 클래스는 을 제공합니다. 테마 변경 사항을 구독하는 NET 친화적 인 이벤트. `ThemeChanged` 이벤트를 사용하여 새 처리기를 추가하고 메서드를 `GetThemedColor()` 사용하여 관심 있는 색상 값을 `ThemeResourceKeys` 가져옵니다. 샘플 코드는 다음과 같습니다.
...
- 색칠하는 요소와 **의미 체계 의미가 같은 시스템 색상을 선택합니다.** 예를 들어 창 내의 텍스트에 대해 대비가 높은 색상을 선택하는 경우 ControlText가 아닌 WindowText를 사용합니다.

- **전경 /배경 쌍을** 함께 선택하거나 색상 선택이 모든 고대비 테마에서 작동할 것이라고 확신하지 못할 것입니다.

- **UI의 어떤 부분이 가장 중요한지 확인하고 콘텐츠 영역이 두드러지도록 합니다.** 색상 색조의 미묘한 차이가 일반적으로 구별되는 세부 사항을 많이 잃게 되므로 다른 콘텐츠 영역에 대한 색상 변형이 없기 때문에 강한 테두리 색상을 사용하여 콘텐츠 영역을 정의하는 것이 일반적입니다.

### <a name="system-color-set"></a>시스템 색상 세트

[WPF 팀 블로그: SystemColors 참조의](https://blogs.msdn.microsoft.com/wpf/2010/11/30/systemcolors-reference/) 표는 시스템 색상 이름의 전체 집합과 각 테마에 표시되는 해당 색조를 나타냅니다.
...
![고대비에서 미묘한 세부 사항이 손실되는 방법의 예](../../extensibility/ux-guidelines/media/030303-a_propertieswindow.png "030303-a_PropertiesWindow")<br />고대비에서 미묘한 세부 사항이 손실되는 방법의 예

#### <a name="choosing-text-colors-in-an-editor"></a>편집기에서 텍스트 색상 선택

색상이 있는 텍스트는 편집기 나 디자인 표면에 사용되어 유사한 항목 그룹을 쉽게 식별할 수 있도록 하는 것과 같이 의미를 나타냅니다. 그러나 고대비 테마에서는 세 개 이상의 텍스트 색상을 구분할 수 없습니다. 창텍스트, 그레이텍스트 및 핫트랙텍스트는 창백서열 표면에서 사용할 수 있는 유일한 색상입니다. 세 가지 이상의 색상을 사용할 수 없으므로 고대비 모드에서 표시할 가장 중요한 차이점을 신중하게 선택합니다.
...
![고대비 #1 테마의 편집자](../../extensibility/ux-guidelines/media/030303-d_editorhc1.png "030303-d_EditorHC1")<br />고대비 #1 테마의 편집자
...
| 제어 | - 콤보 상자, 드롭 다운 목록, 드롭 다운 버튼을 포함하여 설정 컨트롤 기본 및 비활성화 된 배경, 검색<br />- 도크 대상 버튼 배경<br />- 명령 바 배경<br />- 도구 창 배경 |
...
| 핫트랙 | - 스크롤 바 엄지 손가락 배경과 테두리를 눌러<br />- 누에 스크롤 막대 화살표 문입니다 |
...
| 메뉴 | - 드롭 다운 메뉴 배경<br />- 확인 및 비활성화 된 확인 표시 배경 |
| 메뉴 텍스트 | - 드롭 다운 메뉴 테두리<br />- 체크 표시<br />- 메뉴 문말<br />- 드롭 다운 메뉴 텍스트<br />- 선택한 아이콘 테두리 |
...
코드 편집기 나 디자인 표면을 만들 때와 같이 최종 사용자가 UI를 사용자 지정하도록 허용하는 경우가 있습니다. 이 작업을 수행하는 가장 일반적인 방법은 ** &gt; 도구 옵션** 대화 상자를 사용하는 것입니다. 특수 컨트롤이 필요한 고도로 전문화된 UI가 없는 한 사용자 지정을 표시하는 가장 쉬운 방법은 대화 상자의 **환경** 섹션 내의 **글꼴 및 색상** 페이지를 사용하는 것입니다. 사용자 지정을 위해 노출하는 각 요소에 대해 사용자는 전경 색상, 배경 색 또는 둘 다를 변경하도록 선택할 수 있습니다.

### <a name="building-a-vspackage-for-your-customizable-colors"></a>사용자 정의 가능한 색상을 위한 VS패키지 구축

VSPackage는 사용자 지정 범주를 통해 글꼴 및 색상을 제어하고 글꼴 및 색상 속성 페이지에 항목을 표시할 수 있습니다. 이 메커니즘을 사용하는 경우 VSPackage는 [IVsFontAndColorDefaults공급자](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolordefaultsprovider) 인터페이스와 관련 인터페이스를 구현해야 합니다.

원칙적으로 이 메커니즘은 모든 기존 표시 항목과 해당 항목을 포함하는 범주를 수정하는 데 사용할 수 있습니다. 그러나 텍스트 편집기 범주 또는 표시 항목을 수정하는 데 사용해서는 안 됩니다. 텍스트 편집기 범주에 대한 자세한 내용은 [글꼴 및 색상 개요를](/visualstudio/extensibility/font-and-color-overview?view=vs-2015)참조하십시오.
...
범주의 지역화되지 않은 이름은 `[HKLM\SOFTWARE\Microsoft \Visual Studio\\<Visual Studio version\>\FontAndColors\\<Category\>]` `<Category>` 여기서 특수 유형의 범주 레지스트리 항목을 생성합니다.
...
| 패키지 | REG_SZ | GUID | 범주를 지원하는 VSPackage 서비스의 GUID |
 ...
 레지스트리에 지정된 서비스는 해당 범주에 대한 [IVsFontAndColors의 구현을](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolordefaults) 제공해야 합니다.

#### <a name="to-create-or-identify-groups"></a>그룹을 만들거나 식별하려면

그룹의 지역화되지 않은 이름 `[HKLM\SOFTWARE\Microsoft \Visual Studio\\<Visual Studio version\>\FontAndColors\\<group\>]` 아래에 `<group>` 특수 유형의 범주 레지스트리 항목을 구성합니다.
...
레지스트리에 지정된 서비스는 해당 그룹에 <xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorGroup> 대한 구현을 제공해야 합니다.

![IVsFontAnd컬러 그룹의 구현](../../extensibility/ux-guidelines/media/0304-a_fontandcolorgroup.png "0304-a_FontAndColorGroup")<br />`IVsFontAndColorGroup`의 구현입니다.

### <a name="to-implement-ide-support"></a>IDE 지원을 구현하려면

구현 [GetObject](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolordefaultsprovider.getobject), [iVsFontAndColorDefaults](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolordefaults) 인터페이스 또는 <xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorGroup> 제공 된 각 범주 또는 그룹 GUID에 대 한 IDE에 인터페이스를 반환 합니다.

지원하는 모든 범주에 대해 VSPackage는 [IVsFontAndColors](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolordefaults) 인터페이스의 별도의 인스턴스를 구현합니다.

[IVsFontAndColordefaults를](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolordefaults) 통해 구현 된 방법은 IDE를 제공해야합니다 :
...
IDE는 인터페이스를 <xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorGroup> 사용하여 여러 범주의 결합을 정의합니다.
...
캐시 업데이트는 [IvsFontAndColorCacheManager 인터페이스를](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorcachemanager) 통해 수행되며 전역적으로 또는 선택한 항목에 대해 수행할 수 있습니다.
...
- [은 IVsFontAndColorEvents](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorevents) 인터페이스를 구현하여 **IDE에서 생성된 이벤트를 처리합니다.** IDE는 글꼴 및 색상 페이지를 사용자가 수정한 후 적절한 방법을 호출합니다. 예를 들어 새 글꼴을 선택한 경우 [OnFontChanged](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorevents.onfontchanged) 메서드를 호출합니다.

  **또는**

- **변경 에 대한 IDE를 폴링합니다.** 이것은 시스템 구현 [IVsFontAndColor 저장](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorstorage) 인터페이스를 통해 수행 할 수 있습니다. 주로 지속성을 지원하기 위해 [GetItem](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorstorage.getitem) 메서드는 표시 항목에 대한 글꼴 및 색상 정보를 가져올 수 있습니다. 글꼴 및 색상 설정에 대한 자세한 내용은 [저장된 글꼴 및 색상 설정 에 액세스하는](/visualstudio/extensibility/accessing-stored-font-and-color-settings?view=vs-2015)MSDN 문서를 참조하십시오.

> [!NOTE]
> 폴링 결과가 올바른지 확인하려면 IVsFontAndColorStorage 인터페이스의 검색 방법을 호출하기 전에 [IVsFontAndColorCacheManager](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorcachemanager) 인터페이스를 사용하여 캐시 플러시 및 업데이트가 필요한지 확인합니다. [IVsFontAndColorStorage](/dotnet/api/microsoft.visualstudio.shell.interop.ivsfontandcolorstorage)
...
- `"NameID"`= 패키지에 지역화된 범주 이름의 리소스 ID
- `"ToolWindowPackage"`= 패키지 GUID
- `"Category"="{9FF46859-A47E-47bf-8AC5-EC3DBE69D1FE}"`이는 단지 예일 뿐이며 실제 값은 구현자가 제공하는 새 GUID일 수 있습니다.
