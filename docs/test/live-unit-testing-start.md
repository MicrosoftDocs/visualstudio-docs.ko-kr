...
3. 오른쪽 창의 **클래스 라이브러리(.NET Standard)** 템플릿을 선택하고 다음 이미지와 같이 **이름** 텍스트 상자에 **StringLibrary**를 입력합니다.
...
3. 오른쪽 창의 **단위 테스트 프로젝트(.NET Core)** 템플릿을 선택하고 다음 이미지와 같이 **이름** 텍스트 상자에 **StringLibraryTests**를 입력합니다.
...
5. 단위 테스트 프로젝트는 테스트 중인 클래스 라이브러리에 자동으로 액세스할 수 없습니다. 클래스 라이브러리 프로젝트에 대한 참조를 추가하여 테스트 라이브러리 액세스를 제공합니다. 이렇게 하려면 `StringLibraryTests` 프로젝트를 마우스 오른쪽 단추로 클릭하고 **추가** > **참조**를 선택합니다. **참조 관리자** 대화 상자에서 **솔루션** 탭이 선택되어 있는지 확인하고 다음 이미지에 표시된 것처럼 StringLibrary 프로젝트를 선택합니다.
...
9. **고급 저장 옵션** 대화 상자의 **인코딩** 드롭다운 목록에서 다음 이미지와 같이 **유니코드(서명 없는 UTF-8) - 코드 페이지 65001**을 선택합니다.
...
1. 필요에 따라 StringLibrary에 대한 코드가 포함된 코드 창을 선택합니다. C# 프로젝트의 경우 *Class1.cs*이고, Visual Basic 프로젝트의 경우 *Class1.vb*입니다. (이 단계를 통해 Live Unit Testing을 사용하도록 설정한 후 테스트 결과와 코드 검사를 시각적으로 검사할 수 있습니다.)
...
1. `StartsWithUpper` 메서드에서 `if (String.IsNullOrWhiteSpace(s))`라는 줄에서 녹색 확인 표시를 클릭합니다. 다음 이미지처럼 Live Unit Testing은 코드 줄을 포함하는 세 가지 테스트를 나타내며 모두 성공적으로 실행되었습니다.

   !['If' 조건 문에 대한 코드 검사](media/lut-start/code-coverage-cs1.png)

1. `StartsWithUpper` 메서드에서 `return Char.IsUpper(s[0])`라는 줄에서 녹색 확인 표시를 클릭합니다. 다음 이미지처럼 Live Unit Testing은 코드 줄을 포함하는 두 가지 테스트만 나타내며 모두 성공적으로 실행되었습니다.
...
1. 소스 코드를 수정하면 Live Unit Testing이 새 테스트와 수정된 테스트를 자동으로 실행합니다. 다음 **테스트 탐색기** 이미지처럼 추가한 테스트 2개와 수정한 테스트 1개가 모두 성공했습니다.
...
1. 테스트를 실행하면 다음 이미지와 같이 Live Unit Testing이 `TestHasEmbeddedSpaces` 메서드가 실패했음을 보여 줍니다.
...
1. `HasEmbeddedSpaces` 메서드 서명이 있는 줄 위로 마우스를 가리킵니다. Live Unit Testing은 다음 이미지와 같이 메서드가 하나의 테스트에서 검사됨을 보고하는 도구 설명을 표시합니다.

   ![실패한 테스트에서 Live Unit Testing 정보](media/lut-start/test-failure-info-cs.png)

1. 실패한 **TestHasEmbeddedSpaces** 테스트를 선택합니다. Live Unit Testing은 다음 이미지와 같이 모든 테스트 실행, 선택한 테스트 실행, 모든 테스트 디버깅 및 선택한 테스트 디버깅과 같은 다양한 옵션을 제공합니다.
...
1. Visual Studio는 디버그 모드에서 테스트를 실행합니다.
   ...
   테스트에서는 배열의 각 문자열을 `phrase`라는 변수에 할당하고 이를 `HasEmbeddedSpaces` 메서드에 전달합니다. 어설션 문이 처음으로 `false`일 때 프로그램 실행이 일시 중지되고 디버거를 호출합니다. [`Microsoft.VisualStudio.TestTools.UnitTesting.Assert.IsTrue`](/dotnet/api/microsoft.visualstudio.testtools.unittesting.assert.istrue) 메서드 호출의 예기치 않은 값으로 생성되는 예외 대화 상자가 다음 이미지에 표시됩니다.

   ![Live Unit Testing 예외 대화 상자](media/lut-start/exception-dialog-cs.png)

   또한 다음 이미지와 같이 Visual Studio에서 제공하는 모든 디버깅 도구를 실패한 테스트를 문제 해결하는 데 사용할 수 있습니다.
...
1. Live Unit Testing은 실패한 테스트 메서드를 자동으로 다시 실행하고 다음 이미지와 같이 코드 창과 **테스트 탐색기**에서 결과를 업데이트합니다.
