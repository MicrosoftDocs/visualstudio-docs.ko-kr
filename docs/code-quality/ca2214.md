...
> [!NOTE]
> 이 규칙의 레거시 분석 구현에는** \[ 클래스에 의해 정의 된 가상 메서드를 호출 하는 호출 체인이 포함 된 "생성자 이름"의 진단 메시지가 다릅니다. 의도 하지 않은 결과를 보려면 다음 호출 스택을 검토**하십시오. " 이 규칙의 [FxCop 분석기](install-fxcop-analyzers.md) 구현에는 "**생성자에서 재정의할 수 있는 메서드를 호출 하지**않습니다." 라는 진단 메시지가 있습니다.
