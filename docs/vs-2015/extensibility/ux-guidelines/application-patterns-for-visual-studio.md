...
### <a name="overview"></a>개요
 Visual Studio에서 사용되는 두 가지 주요 창 유형은 문서 편집기와 도구 창입니다. 드물지만 가능한 큰 모덜리스 대화 상자입니다. 이러한 것들은 모두 쉘에서 모덜리스이지만 패턴은 근본적으로 다릅니다. 이 항목에서는 문서 창, 도구 창 및 모데리스 대화 상자간의 차이점을 다룹니다. 모달 대화 상자 패턴은 [대화 상자에서 다룹니다.](../../extensibility/ux-guidelines/application-patterns-for-visual-studio.md#BKMK_Dialogs)
 ...
 **도구 창은** IDE 가장자리에 대해 축소된 표시, 숨김 또는 자동 숨김이 가능한 별도의 작은 창으로 가장 자주 표시됩니다. 그러나 창에서 **Window/Docking** 속성을 선택 취소하여 문서 내에 잘 표시되는 경우가 있습니다. 이로 인해 더 많은 부동산이 발생하지만 일반적인 디자인 결정도 있습니다: Visual Studio에 통합하려고 할 때 피처에 도구 창또는 문서 창을 표시할지 여부를 결정해야 합니다.

 **비주얼 스튜디오에서는 모드없는 대화 상자가** 권장되지 않습니다. 대체로, 그들은 – 정의에 의해 – 부동 도구 창과 같이 구현되어야한다. 모덜리스 대화 상자는 쉘의 측면에 도킹된 일반 도구 창의 크기가 너무 제한적인 경우에 허용됩니다. 또한 사용자가 대화 상자를 보조 모니터로 이동할 가능성이 있는 경우에도 허용됩니다.
...
|**커밋 모델**|*지연된 커밋*<br /><br /> 문서에 데이터를 저장하려면 사용자가 파일/저장, 현재 저장 또는 모두 저장 명령을 수행해야 합니다. 문서 창에는 저장 명령 중 하나에 커밋된 다음 "더럽혀진" 데이터 개념이 있습니다. 문서 창을 닫을 때 모든 내용이 디스크에 저장되거나 손실됩니다.|*즉각적인 커밋*<br /><br /> 저장 모델이 없습니다. 파일 편집에 도움이 되는 검사기 도구 창의 경우 파일은 활성 편집기 또는 디자이너에서 열려 있어야 하며 편집기 또는 디자이너는 저장을 소유합니다.|*지연되거나 즉각적인 커밋*<br /><br /> 대부분의 경우 대규모 모덜리스 대화 상자에서는 변경 내용을 커밋하는 작업이 필요하며 대화 상자 세션 내에서 변경한 내용을 롤백하는 "취소" 작업을 허용합니다.  이렇게 하면 모덜리스 대화 상자가 항상 즉각적인 커밋 모델이 있는 도구 창의 도구 창과 구별됩니다.|
...
- 상위 문서가 활성화된 경우에만 표시되도록 가능하면 문서별 도구 창을 자동으로 표시해야 합니다.
...
- **자동 숨겨진** 도구 창은 고정해제되어 있습니다. 창은 보이지 않는 곳에서 미끄러져 문서 영역 가장자리에 탭(도구 창 이름과 아이콘)을 남길 수 있습니다. 사용자가 탭 위로 마우스를 가져가면 도구 창이 밖으로 미끄러지입니다.
...
- **탭된 문서** 도구 창은 문서 내에 잘 도킹할 수 있습니다. 이 기능은 개체 브라우저와 같은 대형 도구 창에 유용하며 프레임 가장자리에 도킹하는 것보다 더 많은 부동산이 필요합니다.
  ...
  ![Visual Studio의 도구 창 상태](../../extensibility/ux-guidelines/media/0702-01-toolwindowstates.png "01_ToolWindowStates 0702-01_ToolWindowStates")
...
#### <a name="single-instance-and-multi-instance"></a>단일 인스턴스 및 다중 인스턴스
 ...
 ![Visual Studio의 명령을 사용하는 도구 창](../../extensibility/ux-guidelines/media/0702-02-toolwindowenablingcommand.png "02_ToolWindowEnablingCommand 0702-02_ToolWindowEnablingCommand")

 **창 인스턴스가 활성화되어 있을 때 '새 창' 명령을 사용하도록 설정하는 도구 창**
...
|Type|공구 창|함수|
|----------|-----------------|--------------|
|**Hierarchy**|솔루션 탐색기|프로젝트, 기타 파일 및 솔루션 항목에 포함된 문서 목록을 표시하는 계층적 트리입니다. 프로젝트 내의 항목 표시는 프로젝트 유형을 소유하는 패키지(예: 참조 기반, 디렉터리 기반 또는 혼합 모드 유형)에 의해 정의됩니다.|
|**Hierarchy**|클래스 뷰|파일 자체와 는 별개로 클래스의 계층적 트리와 문서 작업 집합의 다양한 요소입니다.|
|**Hierarchy**|서버 탐색기|솔루션의 모든 서버 및 데이터 연결을 표시하는 계층적 트리입니다.|
|**Hierarchy**|문서 개요|활성 문서의 계층 구조입니다.|
|**표 형태**|속성|선택한 객체에 대한 속성 목록과 해당 속성을 편집할 값 선택기를 표시하는 그리드입니다.|
|**표 형태**|작업 목록|사용자가 작업 및 주석을 생성/편집/삭제할 수 있는 그리드입니다.|
|**콘텐츠**|도움말|사용자가 "어떻게 해야 합니까?" 에서 도움을 받는 다양한 방법에 액세스할 수 있는 창입니다. MSDN 포럼에 비디오를 제공합니다.|
|**콘텐츠**|동적 도움말|현재 선택 영역에 적용할 수 있는 데 도움이 되는 항목에 대한 링크를 표시하는 도구 창입니다.|
|**콘텐츠**|개체 브라우저|왼쪽 창에 계층적 개체 구성 요소 목록과 오른쪽 열에 개체의 속성 및 메서드가 있는 2열 프레임 집합입니다.|
|**대화 상자**|찾기, 고급 찾기|사용자가 솔루션 내의 다양한 파일을 찾거나 찾고 바꿀 수 있는 대화 상자입니다.|
|**기타**|도구 상자|설계 표면에 삭제되는 요소를 저장하는 데 사용되는 도구 창은 모든 디자이너에게 일관된 드래그 소스를 제공합니다.|
|**기타**|시작 페이지|개발자 뉴스, Visual Studio 도움말 및 최근 프로젝트의 피드에 액세스할 수 있는 Visual Studio에 대한 사용자의 포털입니다. 또한 사용자는 "Common7\IDE\StartPages\" Visual Studio 프로그램 파일 디렉토리에서 "Common7\IDE\StartPages\" 파일 디렉토리를 Visual Studio 문서 디렉토리의 StartPages 폴더에 복사한 다음 XAML을 손으로 편집하거나 직접 편집하여 사용자 지정 시작 페이지를 만들 수도 있습니다. 비주얼 스튜디오 또는 다른 코드 편집기에서 열 수 있습니다.|
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|자동||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|즉시||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|출력|출력 창은 선언할 텍스트 이벤트 또는 상태가 있을 때마다 사용할 수 있습니다.|
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|메모리||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|중단점||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|실행 중||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|문서||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|호출 스택||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|로컬||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|시계||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|디스어셈블리||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|레지스터||
|**디버거:** 작업 및 모니터링 활동과 관련된 창 그룹|스레드||
...
- 메뉴 명령은 **편집,** **형식**및 **보기** 메뉴와 같은 공통 메뉴에 적절하게 통합됩니다. 상당한 양의 특수 명령을 사용할 수 있는 경우 문서에 포커스가 있는 경우에만 표시되는 새 메뉴를 만들 수 있습니다.
...
3. 상호 작용 및 레이아웃 지침에 따라 새 대화 상자를 만듭니다.
   ...
   이 항목에서는 Visual Studio 워크플로 및 대화 디자인에 대한 일반적인 규칙 내에서 올바른 대화 상자 패턴을 선택하는 방법을 설명합니다.

### <a name="themes"></a>테마
 Visual Studio의 대화 상자는 두 가지 기본 스타일 중 하나를 따릅니다.

#### <a name="standard-unthemed"></a>표준(테마가 없는)
 대부분의 대화 상자는 표준 유틸리티 대화 상자이며 테마를 해제해야 합니다. 일반적인 컨트롤을 다시 템플릿으로 만들거나 양식에 일치시키는 "최신" 단추 또는 컨트롤을 만들려고 시도하지 마십시오. 컨트롤 및 크롬 [모양은 대화 상자에 대한 표준 Windows 데스크톱 상호 작용 지침을 따릅니다.](https://msdn.microsoft.com/library/windows/desktop/dn742499\(v=vs.85\).aspx)
...
- 대화 상자는 제품 버전을 홍보하거나 차별화하는 데 전략적역할을 하는 경험의 중요한 부분입니다.
  ...
  테마 대화 상자를 만들 때 적절한 환경 색상을 사용하고 올바른 레이아웃 및 상호 작용 패턴을 따릅니다. (비주얼 [스튜디오용 레이아웃](../../extensibility/ux-guidelines/layout-for-visual-studio.md)참조)
 ...
 **강력한 이름 만들기 키는 Visual Studio에서 간단한 대화 상자의 예입니다.**
 ...
 **옵션 > 도구** 대화 상자는 포함된 트리를 사용하여 계층화된 대화 상자의 예입니다.

 ![Visual Studio의 계층화된 대화 상자](../../extensibility/ux-guidelines/media/0704-02-toolsoptions.png "02_ToolsOptions 0704-02_ToolsOptions")

 **도구 > 옵션은 Visual Studio의 계층화된 대화 상자의 예입니다.**
...
#### <a name="position"></a>위치
 대화 상자는 처음 시작할 때 IDE 내에서 중앙에 나타나야 합니다. 재조정 불가능한 대화 상자의 경우 대화 상자의 마지막 위치를 유지해야 할 필요가 없으므로 후속 시작 중심으로 표시됩니다. 크기를 조정할 수 있는 대화 상자의 경우 이후 시작 시 크기를 유지해야 합니다. 모달인 재지정 가능한 대화 상자의 경우 위치를 유지하지 않아도 됩니다. IDE 가운데에 표시하면 사용자의 디스플레이 구성이 변경될 때 대화 상자가 예기치 않거나 사용할 수 없는 위치에 나타날 가능성을 방지합니다. 재배치할 수 있는 모덜리스 대화 상자의 경우 대화 상자가 더 큰 워크플로의 필수적인 부분으로 자주 사용될 수 있기 때문에 후속 시작 시 사용자의 위치를 유지해야 합니다.

 대화 상자가 다른 대화 상자를 생성해야 하는 경우 맨 위 대화 상자가 부모에서 오른쪽아래로 계단식으로 배열되어 사용자가 새 곳으로 이동한 것을 분명히 해야 합니다.
...
- 제목 표시줄의 도움말 버튼이 더 이상 사용되지 않습니다. 새 대화 상자에 추가하지 마십시오. 존재할 때 는 작업과 개념적으로 관련된 도움말 항목을 시작해야 합니다.
  ...
  **Visual Studio 대화 상자의 제목 표시줄에 대한 지침 사양입니다.**

#### <a name="control-buttons"></a>제어 버튼
 일반적으로 **도움말**/**취소**/**Help** 단추는 대화 상자의 오른쪽 아래 모서리에 가로로 정렬되어야 합니다. 대화 상자에 컨트롤 단추와 시각적으로 혼동을 줄 수 있는 대화 상자 의 맨 아래에 여러 개의 다른 단추가 있는 경우 대체 수직 스택이 허용됩니다.

 ![Visual Studio의 컨트롤 단추 구성](../../extensibility/ux-guidelines/media/0704-04-controlbuttonconfig.png "04_ControlButtonConfig 0704-")

 **Visual Studio 대화 상자의 제어 단추에 허용되는 구성**
 ...
 **확인**/**도움말** **취소**/버튼에 액세스 키를 사용하지 마십시오. 이러한 단추는 기본적으로 바로 가기 키에 매핑됩니다.
...
|-------------------------|------------------------------------|-----------------------------------------|
|탭 컨트롤|대화 상자를 관련 집합으로 논리적으로 그룹화<br /><br /> 대화 상자에서 관련 컨트롤의 5개 미만(또는 대화 상자에서 한 행에 맞는 탭 수) 페이지에 유용합니다.<br /><br /> 탭 레이블은 짧아야 합니다: 콘텐츠를 쉽게 식별할 수 있는 단어 하나 또는 두 개<br /><br /> 일반적인 시스템 대화 상자 스타일<br /><br /> 예: **파일 탐색기 > 항목 속성**|설명이 있는 짧은 레이블을 만드는 것은 어려울 수 있습니다.<br /><br /> 일반적으로 하나의 대화 상자에서 5개의 탭을 지나확장하지 않습니다.<br /><br /> 한 행에 탭이 너무 많으면 부적절합니다. 대체 레이어링 기법 사용<br /><br /> 확장 할 수 없습니다|
...
|마법사|작업 기반의 순차적 단계를 안내하여 작업 완료를 지원합니다. 마법사는 상위 수준의 작업을 나타내며 개별 패널은 전체 작업을 수행하는 데 필요한 하위 작업을 나타냅니다.<br /><br /> 사용자가 작업을 완료하기 위해 여러 편집기 와 도구 창을 사용해야 하는 경우와 같이 작업이 Ui 경계를 넘을 때 유용합니다.<br /><br /> 작업에 분기가 필요한 경우 유용합니다.<br /><br /> 작업에 단계 간 종속성이 포함된 경우 유용합니다.<br /><br /> 하나의 결정 포크가 있는 여러 유사한 작업을 하나의 대화 상자에 표시하여 서로 다른 유사한 대화 상자의 수를 줄일 수 있는 경우에 유용합니다.|순차워크플로우가 필요하지 않은 작업에 적합하지 않음<br /><br /> 사용자가 너무 많은 단계로 마법사에 의해 압도되고 혼동 될 수 있습니다.<br /><br /> 마법사는 본질적으로 화면 부동산이 제한되어 있습니다.|
 ...
 **Outlook에서 추가 UI를 노출하기위한 복도 개념**

##### <a name="adaptive-ui"></a>적응형 UI
 사용 또는 사용자의 자체 보고 환경을 기반으로 UI를 표시하거나 숨기는 것은 다른 부분을 숨기면서 필요한 UI를 표시하는 또 다른 방법입니다. UI를 표시하거나 숨길 시기를 결정하는 알고리즘은 까다로울 수 있으며 일부 사례에서는 항상 규칙이 잘못될 수 있기 때문에 Visual Studio에서는 권장되지 않습니다.
...
### <a name="drag-and-drop-interaction-model"></a>끌어서 놓기 상호 작용 모델
 프로젝트는 일반적으로 자신을 참조 기반(저장소의 프로젝트 항목에 대한 참조만 지속 가능), 디렉터리 기반(프로젝트 계층 내에 물리적으로 저장된 프로젝트 항목만 유지가능) 또는 혼합(참조를 지속할 수 있음)으로 분류합니다. 또는 물리적 항목). IDE는 **솔루션 탐색기**내에서 세 가지 유형의 프로젝트를 동시에 수용합니다.
...
- 참조 기반 및 혼합 프로젝트 시스템은 원본 항목에 링크(또는 참조)를 추가하는 개념을 지원합니다. 이러한 프로젝트가 끌어서 놓기 작업의 대상인 **경우(Ctrl + Shift가** 유지될 때) 프로젝트에 추가되는 항목에 대한 참조가 생성되어야 합니다.
  ...
  끌어서 놓기 동작을 제대로 구현하려면 드래그의 소스 프로젝트가 특성(예: 참조 또는 디렉터리 기반입니까?)을 대상 프로젝트에 전달해야 합니다. 이 정보는 소스에서 제공하는 클립보드 형식으로 표시됩니다. 드래그(또는 클립보드 복사 작업)의 소스로서 프로젝트는 프로젝트가 참조 기반인지 디렉터리 기반인지에 따라 각각 **CF_VSREFPROJECTITEM**S 또는 **CF_VSSTGPROJECTITEMS** 제공해야 합니다. 두 형식 모두 동일한 데이터 콘텐츠를 가지고 있으며, 이는 파일 이름이 아닌 문자열 목록이 되는 것을 제외하고는 Windows **CF_HDROP** 형식과 유사하며, **Projref** 문자열의 이중**NULL** 종료 목록입니다(IVsSolution:GetProjrefOfItem 또는 **:GetProjrefOfProject에서** 반환됨). **IVsSolution::GetProjrefOfItem**

  드롭(또는 클립보드 붙여넣기 작업)의 대상으로 프로젝트는 **CF_VSREFPROJECTITEMS** **CF_VSSTGPROJECTITEMS**모두 허용해야 하지만 끌어서 놓기 작업의 정확한 처리는 대상 프로젝트와 소스 프로젝트의 특성에 따라 다릅니다. 소스 프로젝트는 **CF_VSREFPROJECTITEMS** **제공여부에**따라 CF_VSSTGPROJECTITEMS. 드롭의 대상은 자체 특성을 이해하므로 이동, 복사 또는 링크를 수행할지 여부를 결정할 수 있는 충분한 정보가 있습니다. 또한 사용자는 Ctrl, Shift 또는 Ctrl 및 Shift 키를 모두 눌러 드래그 앤 드롭 작업을 수행해야 하는 작업을 수정합니다. 끌어놓기 대상이 **DragEnter** 및 **DragOver** 메서드에서 미리 수행될 작업을 적절하게 표시하는 것이 중요합니다. **솔루션 탐색기는** 소스 프로젝트와 대상 프로젝트가 동일한 프로젝트인지 여부를 자동으로 알고 있습니다.
...
|-------------------|-------------|-----------------|
|![마우스 "끌어 놓기 없음" 아이콘](../../extensibility/ux-guidelines/media/0706-01-mousenodrop.png "0706-01_MouseNoDrop")|드롭 없음|지정된 위치로 항목을 삭제할 수 없습니다.|
|![마우스 "복사" 아이콘](../../extensibility/ux-guidelines/media/0706-02-mousecopy.png "0706-02_MouseCopy")|복사|항목이 대상 위치로 복사됩니다.|
|![마우스 "이동" 아이콘](../../extensibility/ux-guidelines/media/0706-03-mousemove.png "0706-03_MouseMove")|이동|항목이 대상 위치로 이동됩니다.|
|![마우스 "참조 추가" 아이콘](../../extensibility/ux-guidelines/media/0706-04-mouseaddref.png "0706-04_MouseAddRef")|추가 참조|선택한 항목에 대한 참조가 대상 위치에 추가됩니다.|
...
|||원본 항목: 참조/링크|원본 항목: 물리적 항목 또는 파일 시스템(CF_HDROP)|
|-|-|----------------------------------|-------------------------------------------------------------|
|수정자 없음|작업|이동|링크|
|수정자 없음|대상|원본 항목에 참조 추가|원본 항목에 참조 추가|
|수정자 없음|원본|원본 항목에 대한 참조 삭제|원래 항목 유지|
|수정자 없음|결과|**DROPEFFECT_MOVE** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_LINK** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|
|시프트+드래그|작업|이동|드롭 없음|
|시프트+드래그|대상|원본 항목에 참조 추가|드롭 없음|
|시프트+드래그|원본|원본 항목에 대한 참조 삭제|드롭 없음|
|시프트+드래그|결과|**DROPEFFECT_MOVE** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|드롭 없음|
|Ctrl+드래그|작업|복사|드롭 없음|
|Ctrl+드래그|대상|원본 항목에 참조 추가|드롭 없음|
|Ctrl+드래그|원본|원본 항목에 대한 참조 유지|드롭 없음|
|Ctrl+드래그|결과|**DROPEFFECT_COPY** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|드롭 없음|
|Ctrl+시프트+드래그|작업|링크|링크|
|Ctrl+시프트+드래그|대상|원본 항목에 참조 추가|원본 항목에 참조 추가|
|Ctrl+시프트+드래그|원본|원본 항목에 대한 참조 유지|원래 항목 유지|
|Ctrl+시프트+드래그|결과|**DROPEFFECT_LINK** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_LINK** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|
...
|수정자 없음|결과|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|
|시프트+드래그|작업|이동|이동|
|시프트+드래그|대상|대상 위치에 항목 복사|대상 위치에 항목 복사|
|시프트+드래그|원본|원본 항목에 대한 참조 삭제|원래 위치에서 항목을 삭제합니다.|
|시프트+드래그|결과|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|
|Ctrl+드래그|작업|복사|복사|
|Ctrl+드래그|대상|대상 위치에 항목 복사|대상 위치에 항목 복사|
|Ctrl+드래그|원본|원본 항목에 대한 참조 유지|원본 항목에 대한 참조 유지|
|Ctrl+드래그|결과|**DROPEFFECT_ COPY는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_ COPY는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|
...
|수정자 없음|원본|원본 항목에 대한 참조 삭제|원본 항목에 대한 참조 삭제|
|수정자 없음|결과|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되고 항목은 저장소의 원래 위치에서 삭제됩니다.|
|시프트+드래그|작업|이동|이동|
|시프트+드래그|대상|원본 항목에 참조 추가|대상 위치에 항목 복사|
|시프트+드래그|원본|원본 항목에 대한 참조 삭제|원래 위치에서 항목을 삭제합니다.|
|시프트+드래그|결과|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_ MOVE는** **::Drop에서** 작업으로 반환되고 항목은 저장소의 원래 위치에서 삭제됩니다.|
|Ctrl+드래그|작업|복사|복사|
...
|Ctrl+시프트+드래그|원본|원본 항목에 대한 참조 유지|원래 항목 유지|
|Ctrl+시프트+드래그|결과|**DROPEFFECT_ LINK는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|**DROPEFFECT_ LINK는** **::Drop에서** 작업으로 반환되며 항목은 저장소의 원래 위치에 남아 있습니다.|
|잘라내기/붙여넣기|작업|이동|이동|
|잘라내기/붙여넣기|대상|대상 위치에 항목 복사|대상 위치에 항목 복사|
|잘라내기/붙여넣기|원본|원본 항목에 대한 참조 삭제|원래 위치에서 항목을 삭제합니다.|
|잘라내기/붙여넣기|결과|항목은 저장소의 원래 위치에 남아 있습니다.|저장소의 원래 위치에서 항목이 삭제됩니다.|
...
2. 열려 있는 편집기/디자이너에 저장되지 않은 변경 내용이 있는 경우 드래그 소스는 드롭이 발생할 때까지 기다린 다음 다음과 유사한 프롬프트로 창을 닫기 전에 열려 있는 문서에서 커밋되지 않은 변경 내용을 저장하도록 사용자에게 요청해야 합니다.
   ...
   이렇게 하면 대상이 복사본을 만들기 전에 진행 중인 작업을 저장할 수 있습니다. 이 처리를 활성화하기 위해 새로운 방법 **IVsHierarchyDropDataSource2::OnBeforeDropNotify가** 추가되었습니다.

   그런 다음 대상이 저장소에 있는 항목의 상태를 복사합니다(사용자가 **아니요를**선택한 경우 편집기에서 저장되지 않은 변경 내용을 포함하지 않음). 대상이 복사를 완료한 **후(iVsHierarchyDropDataSource::Drop에서)** 소스에는 이동 작업의 삭제 부분을 완료할 수 있는 기회가 주어집니다(IVsHierarchyDropDataSource::OnDropNotify). **IVsHierarchyDropDataSource::OnDropNotify**
