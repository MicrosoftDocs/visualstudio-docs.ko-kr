...
1. *Dynamicitemmenucommand. cs* 라는 새 c # 클래스 파일을 만들고에서 상속 된 **dynamicitemmenucommand** 라는 클래스를 추가 합니다 <xref:Microsoft.VisualStudio.Shell.OleMenuCommand> .
...
1. *DynamicMenuPackage* 에서 명령 집합의 GUID와 명령 ID를 추가 합니다.
