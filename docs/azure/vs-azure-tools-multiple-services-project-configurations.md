...
- `ServiceDefinition.csdef`는 클라우드 서비스 및 해당 역할의 요구 사항을 설명하고 모든 인스턴스에 적용되는 설정을 제공하기 위해 Azure에 배포됩니다. Azure 서비스 호스팅 런타임 API를 사용하여 런타임에 설정을 읽을 수 있습니다. 이 파일은 클라우드 서비스를 중지하는 경우에만 Azure에서 업데이트할 수 있습니다.
...
**설정** 페이지에서 설정을 구성에 이름-값 쌍으로 추가할 수 있습니다. 역할에서 실행 중인 코드는 [Azure 관리 라이브러리,](/previous-versions/azure/dn602775(v=azure.11))특히 [GetConfigurationValue](/previous-versions/azure/reference/ee772857(v=azure.100)) 메서드에서 제공하는 클래스를 사용하여 런타임에 구성 설정값을 읽을 수 있습니다.
