...
## <a name="code-access-security-policies"></a>코드 액세스 보안 정책
 응용 프로그램에 대한 사용 권한은 응용 프로그램 매니페스트의 [ \<trustInfo> 요소](../deployment/trustinfo-element-clickonce-application.md) 요소 요소의 설정에 의해 결정됩니다. [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]은 프로젝트의 **보안** 속성 페이지의 설정에 따라 이 정보를 자동으로 생성합니다. [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] 애플리케이션에는 요청된 특정 권한만 부여됩니다. 예를 들어, 파일에 액세스하는 데 완전 신뢰 권한이 필요할 때 애플리케이션에서 파일 액세스 권한을 요청하면 완전 신뢰 권한이 아니라 파일 액세스 권한만 부여됩니다. [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] 애플리케이션을 개발할 때, 애플리케이션에서 필요한 특정 권한만 요청해야 합니다. 대부분의 경우, 인터넷 또는 로컬 인트라넷 영역을 사용하여 애플리케이션을 부분 신뢰로 제한할 수 있습니다. 자세한 내용은 [클릭Once 응용 프로그램에 대한 보안 영역 설정 방법: 를](../deployment/how-to-set-a-security-zone-for-a-clickonce-application.md)참조하십시오. 애플리케이션에 사용자 지정 권한이 필요한 경우 사용자 지정 영역을 만들 수 있습니다. 자세한 내용은 [클릭Once 응용 프로그램에 대한 사용자 지정 사용 권한 설정 방법을](../deployment/how-to-set-custom-permissions-for-a-clickonce-application.md)참조하십시오.
 ...
 개발자는 적절한 사용 권한으로 애플리케이션이 실행되도록 해야 합니다. 애플리케이션이 런타임에 영역 외부에서 권한을 요청하는 경우 보안 예외가 발생할 수 있습니다. [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 는 대상 보안 영역에서 애플리케이션을 디버깅할 수 있도록 합니다. 또한, 보안 애플리케이션 개발에 대한 도움말을 제공 합니다. 자세한 내용은 [사용 방법: 제한된 권한이 있는 ClickOnce 응용 프로그램 디버그를](securing-clickonce-applications.md)참조하십시오.

 코드 액세스 보안 및 ClickOnce에 대한 자세한 내용은 [ClickOnce 응용 프로그램에 대한 코드 액세스 보안을](../deployment/code-access-security-for-clickonce-applications.md)참조하십시오.
