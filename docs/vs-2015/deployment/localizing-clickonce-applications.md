...
---
# <a name="localizing-clickonce-applications"></a>ClickOnce 애플리케이션 지역화
...
지역화는 애플리케이션을 특정 문화권에 적합하게 만드는 프로세스입니다. 이 프로세스에서는 UI(사용자 인터페이스) 텍스트를 지역별 언어로 번역하고, 올바른 데이터 및 통화 형식 지정을 사용하고, 양식에서 컨트롤 크기를 조정하고, 필요하면 오른쪽에서 왼쪽으로 컨트롤을 미러링합니다.  
 ...
 이 항목에서는 다른 문화권에 맞게 [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] 애플리케이션을 배포하는 세 가지 방법을 설명합니다.  
...
## <a name="including-all-satellite-assemblies-in-a-deployment"></a>배포에 모든 위성 어셈블리 포함  
 ...
 MageUI.exe에서이 메서드를 사용 하려면 MageUI.exe에서 **중립** 으로 응용 프로그램에 대 한 문화권을 설정 해야 합니다. 그 후에 모든 위성 어셈블리를 배포에 수동으로 포함해야 합니다. MageUI.exe에서는 응용 프로그램 매니페스트의 **파일** 탭에서 **채우기** 단추를 사용 하 여 위성 어셈블리를 추가할 수 있습니다.  
...
## <a name="generate-one-deployment-for-each-culture"></a>각 문화권에 대해 하나의 단일 배포 생성  
 ...
 Microsoft [!INCLUDE[winsdklong](../includes/winsdklong-md.md)]에서 MageUI.exe 도구를 사용하여 같은 작업을 수행할 수 있습니다. 응용 프로그램 매니페스트의 **파일** 탭에서 **채우기** 단추를 사용 하 여 응용 프로그램 디렉터리에서 다른 모든 위성 어셈블리를 제외 하 고 MageUI.exe의 배포 매니페스트에 대 한 **이름** 탭에서 **문화권** 필드를 설정 합니다. 이러한 단계에서는 올바른 위성 어셈블리를 포함하고 배포 매니페스트의 `assemblyIdentity` 요소에서 `language` 특성을 해당 문화권으로 설정합니다.  
...
## <a name="downloading-satellite-assemblies-on-demand"></a>요청 시 위성 어셈블리 다운로드  
 ...
 요청 시 위성 어셈블리 다운로드는 요청 시 기타 유형 어셈블리를 다운로드하는 것과 약간 다릅니다. 사용 하 여이 시나리오를 사용 하도록 설정 하는 방법에 자세한 내용과 코드 예제는 [!INCLUDE[winsdkshort](../includes/winsdkshort-md.md)] 에 대 한 도구 [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)]를 참조 하세요 [연습: ClickOnce 배포 API에서 요청 시 위성 어셈블리 다운로드](../deployment/walkthrough-downloading-satellite-assemblies-on-demand-with-the-clickonce-deployment-api.md).  
  
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]에서 이 시나리오를 사용하도록 설정할 수도 있습니다.  또한 [연습: 디자이너를 사용하여 ClickOnce 배포 API에서 요청 시 위성 어셈블리 다운로드](https://msdn.microsoft.com/library/ms366788\(v=vs.110\)) 또는 [연습: 디자이너를 사용하여 ClickOnce 배포 API에서 요청 시 위성 어셈블리 다운로드](https://msdn.microsoft.com/library/ms366788\(v=vs.120\))를 참조하세요.  
...
- 해당하는 지역화 버전 Windows에서 [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] 애플리케이션을 실행할 수 있습니다.  
