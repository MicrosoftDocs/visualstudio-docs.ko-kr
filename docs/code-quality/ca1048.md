...
description: Public 형식은 sealed 이며 최종이 아닌 가상 (Visual Basic의 재정의 가능) 인 메서드를 선언 합니다. 이 규칙은이 패턴을 따라야 하는 대리자 형식에 대 한 위반을 보고 하지 않습니다.
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
Visual Basic 및 c # 컴파일러에서는 형식이이 규칙을 위반 하는 것을 허용 하지 않습니다.
