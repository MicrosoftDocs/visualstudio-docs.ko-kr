...
---
# <a name="watch-variables-with-watch-windows-and-quickwatch"></a>시계 창 및 퀵워치로 변수 보기

디버깅하는 동안 **Watch** 창 및 **QuickWatch를** 사용하여 변수 및 식을 볼 수 있습니다. 창은 디버깅 세션 중에만 사용할 수 있습니다.

**시계** 창은 디버깅하는 동안 한 번에 여러 변수를 표시할 수 있습니다. **QuickWatch** 대화 상자에는 한 번에 하나의 변수가 표시되며 디버깅을 계속하기 전에 닫아야 합니다.
...
예를 들어 `a`의 `b`값과 `c` 다음 코드에서 시계를 설정합니다.
...
1. 왼쪽 여백을 클릭하거나 **디버그** > **토글 중단점을**선택하거나 **F9을**눌러 `c = a + b;` 줄에 중단점을 설정합니다.

1. 녹색 **시작** 화살표 또는 **디버그** > **시작 디버깅을**선택하여 디버깅을 시작하거나 **F5를**누릅니다. 중단점에서 실행이 일시 중지됩니다.

1. **디버그** > **윈도우** > **시계** > **1을**선택하거나 **Ctrl**+**Alt**+**W** > **1을**눌러 **시계** 창을 엽니다.

   창 **2,** **3**또는 **4를**선택하여 추가 **보기 창을** 열 수 있습니다.

1. **Watch** 창에서 빈 행을 선택하고 변수를 `a`입력합니다. 에 대해 `b` 동일한 `c`작업을 수행합니다.

   ![보기 변수](../debugger/media/watchvariables.png "시계 변수")

1. **디버그** > **스텝 인을** 선택하거나 필요에 따라 **F11을** 눌러 디버깅을 계속합니다. **루프를** 반복할 때 Watch 창의 `for` 변수 값이 변경됩니다.

>[!NOTE]
>C+만의 경우
>- 변수 이름 또는 변수 이름을 사용하는 식의 컨텍스트를 한정해야 할 수 있습니다. 컨텍스트는 변수가 있는 함수, 소스 파일 또는 모듈입니다. 컨텍스트를 한정해야 하는 경우 **Watch** 창의 **이름에** [컨텍스트 연산자(C++)](../debugger/context-operator-cpp.md) 구문을 사용합니다.
>
>- 등록 이름 ** $ \<>&nbsp;** 사용하여 등록 이름과 변수 이름을 추가하거나 **Name** **Watch** ** @ \<&nbsp;** Watch 창의 이름에>이름을 등록할 수 있습니다. 자세한 내용은 [Pseudovariables](../debugger/pseudovariables.md)를 참조하세요.

## <a name="use-expressions-in-a-watch-window"></a>감시 창에서 표현식 사용

**감시** 창에서 디버거가 인식하는 유효한 식을 관찰할 수 있습니다.

예를 들어 앞섹션의 코드의 경우 `(a + b + c) / 3` **Watch** 창을 입력하여 세 값의 평균을 얻을 수 있습니다.

![시청 식](../debugger/media/watchexpression.png "시청 식")

**Watch** 창에서 식을 평가하는 규칙은 일반적으로 코드 언어의 식을 평가하기 위한 규칙과 동일합니다. 식에 구문 오류가 있는 경우 코드 편집기와 동일한 컴파일러 오류를 예상합니다. 예를 들어, 앞의 식의 오타에서는 **Watch** 창에서 이 오류가 발생합니다.
...
각 창 위의 검색 창을 사용하여 **Watch** 창의 이름, 값 및 유형 열에서 키워드를 검색할 수 있습니다. ENTER를 누르거나 화살표 중 하나를 선택하여 검색을 실행합니다. 진행 중인 검색을 취소하려면 검색 표시줄에서 "x" 아이콘을 선택합니다.

왼쪽 및 오른쪽 화살표(각각 Shift+F3 및 F3)를 사용하여 찾은 일치 항목 사이를 탐색합니다.

![시계 창에서 검색](../debugger/media/ee-search-watch.png "시계 창에서 검색")

검색을 더 철저하게 만들려면 **조사창** 상단의 **심층** 검색 드롭다운을 사용하여 중첩된 개체로 검색할 수준(깊이)을 선택합니다. 
...
표현식이 평가될 때 새로 고침 아이콘(원형 화살표)이 **Watch** 창에 나타날 수 있습니다. 새로 고침 아이콘은 오류 또는 오래된 값을 나타냅니다.
...
속성 및 암시적 함수 호출의 자동 평가가 비활성화되어 새로 고침 아이콘이 나타나면 **Tools** > **옵션** > **디버깅** > **일반에서**속성 평가 및 기타 **암시적 함수 호출 을** 활성화하여 활성화할 수 있습니다.

새로 고침 아이콘을 사용하여 시연하려면 다음을 수행하십시오.

1. **도구** > **옵션** > **디버깅** > **일반에서**Enable 속성 평가 및 기타 **암시적 함수 호출** 확인란을 선택 취소합니다.

1. 다음 코드를 입력하고 **Watch** 창에서 속성에 시계를 `list.Count` 설정합니다.
...
1. 디버깅을 시작합니다. **보기** 창에는 다음과 같은 메시지가 표시됩니다.
...
일부 식을 평가하면 변수 값이 변경되거나 앱 상태에 영향을 줄 수 있습니다. 예를 들어 다음 식을 계산하면 `var1`의 값이 변경됩니다.

```csharp
var1 = var2
```

이 코드는 [부작용을](https://en.wikipedia.org/wiki/Side_effect_\(computer_science\))일으킬 수 있습니다. 부작용으로 인해 앱 작동 방식을 변경하여 디버깅이 더 어려워질 수 있습니다.
...
C#만의 경우 속성 또는 암시적 함수 호출의 평가가 꺼져 있는 경우 **Watch** 창의 변수 **Name에** **ac** 형식 수정자를 추가하여 평가를 강제할 수 있습니다. [C#의 형식 지정자](../debugger/format-specifiers-in-csharp.md)를 참조하세요.
...
메서드에서 이름을 찾으려면 **Watch** 창에서 `Person` 개체 ID에 대한 참조를 추가할 수 있습니다. `Person` `DoSomething()`

1. 개체를 만든 후 코드에서 `Person` 중단점을 설정합니다.

1. 디버깅을 시작합니다.

1. 중단점에서 실행이 일시 중지되면 **디버그** > **Windows** > 지역**구시가지에서**를 선택하여 **지역 창을** 엽니다.

1. 지역 **변수** 창에서 변수를 `Person` 마우스 오른쪽 단추로 클릭하고 **개체 ID 만들기를**선택합니다.

   개체 ID인 지역**$** **창에** 달러 기호 () 및 숫자와 숫자가 표시됩니다.

1. 개체 ID를 마우스 오른쪽 단추로 클릭하고 **감시**추가를 선택하여 개체 ID를 **보기** 창에 추가합니다.

1. 메서드에서 다른 중단점을 설정합니다. `DoSomething()`

1. 디버깅을 계속합니다. 메서드에서 실행이 일시 중지되면 Watch `Person` 창에 개체가 표시됩니다. **Watch** `DoSomething()`

   > [!NOTE]
   > 와 `Person.Name`같은 개체의 속성을 보려면 **Tools** > **옵션** > **디버깅** > **일반** > Enable 속성 평가 및 기타**암시적 함수 호출을**선택하여 속성 평가를 사용하도록 설정해야 합니다.
...
**Watch** 창에는 이러한 개체가 동적 개체로 표시되며 <xref:System.Dynamic.IDynamicMetaObjectProvider> 인터페이스를 구현하는 형식에서 만들어집니다. 동적 개체 노드는 동적 개체의 동적 멤버를 표시하지만 멤버 값의 편집은 허용하지 않습니다.
...
1. 동적 보기의 자식을 마우스 오른쪽 단추로 **클릭합니다.**
1. **시계 추가를**선택합니다. `object.name` 이가 `((dynamic) object).name` 되어 새 **보기** 창에 나타납니다.
...
## <a name="observe-a-single-variable-or-expression-with-quickwatch"></a>QuickWatch를 통해 단일 변수 또는 표현식 관찰
...
1. **디버그** > **퀵워치를** **선택하고, 시프트**+**F9을**누르거나, 마우스 오른쪽 단추로 클릭하고 **QuickWatch를**선택합니다.

   **퀵워치** 대화 상자가 나타납니다. 변수는 `a` **값** **1이**있는 **식** 상자에 있습니다.

   ![퀵워치 변수](../debugger/media/quickwatchvariable.png "퀵워치 변수")

1. 변수를 사용하여 식을 평가하려면 **식** `a + b` 상자에 와 같은 식을 입력하고 **다시 평가를**선택합니다.

   ![퀵워치 표현식](../debugger/media/quickwatchexpression.png "퀵워치 표현식")

1. **QuickWatch의** 변수 또는 식을 **보기** 창에 추가하려면 **시계 추가를**선택합니다.

1. **빠른 보기** 창을 닫기 위해 **닫기를** 선택합니다. **(QuickWatch는** 모달 대화 상자이므로 열려 있는 한 디버깅을 계속할 수 없습니다.)

1. 디버깅을 계속합니다. **감시** 창에서 변수를 관찰할 수 있습니다.
