---
title: 부하 테스트 실행 설정
description: 부하 테스트마다 여러 실행 설정을 만들어 저장한 다음, 테스트를 실행할 때 특정 설정을 하나 선택하여 사용하는 방법을 알아봅니다.
...
|속성|정의|
|-|----------------|
|**스토리지 유형**|부하 테스트에서 가져온 성능 카운터를 저장하는 방법으로, 옵션은 다음과 같습니다.<br /><br /> -   **데이터베이스** - **부하 테스트 결과 저장소** 가 포함된 SQL 데이터베이스가 필요합니다.<br />-   **없음**.|
