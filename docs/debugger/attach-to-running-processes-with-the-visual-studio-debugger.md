...
원격 컴퓨터에서 프로세스를 디버그하려면 [원격 컴퓨터의 프로세스에 연결](#BKMK_Attach_to_a_process_on_a_remote_computer)을 참조하세요.

::: moniker range=">= vs-2019"
Linux Docker 컨테이너에서 .NET Core 프로세스를 디버그하려면 [Linux Docker 컨테이너에 연결](#BKMK_Linux_Docker_Attach)을 참조하세요.
::: moniker-end
...
1. Visual Studio에서 **디버그** > **프로세스에 연결**(또는 **Ctrl**+**Alt**+**P**)을 선택하여 **프로세스에 연결** 대화 상자를 엽니다.
   ...
   **연결 형식**은 **기본**으로 설정해야 합니다. **연결 대상**은 로컬 컴퓨터 이름이어야 합니다.

   ![DBG_Basics_Attach_To_Process](../debugger/media/DBG_Basics_Attach_To_Process.png "DBG_Basics_Attach_To_Process")

2. **사용 가능한 프로세스** 목록에서 연결하려는 프로세스를 찾아 선택합니다.
...
3. **연결 대상** 필드에서 디버그할 코드 형식이 표시되어 있는지 확인합니다. 기본 **자동** 설정은 대부분의 앱 유형에서 작동합니다.
...
2. 대부분의 경우 **연결 형식**은 **기본**이어야 합니다. **연결 대상** 상자에서 다음 방법 중 하나를 사용하여 원격 컴퓨터를 선택합니다.
...
|Linux에서 .NET Core 원격 디버그|**프로세스에 연결** 사용|*dotnet.exe*|SSH를 사용하려면 [SSH를 사용하여 Linux에서 실행되는 .NET Core 원격 디버그](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md)를 참조하세요. |
