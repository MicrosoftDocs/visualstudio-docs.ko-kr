...
MPF (관리 패키지 프레임 워크) 언어 클래스를 사용 하 여에서 언어 서비스를 구현 하 [!INCLUDE[csprcs](../../includes/csprcs-md.md)] 는 것은 간단 합니다. 언어 서비스, 언어 서비스 자체 및 해당 언어의 파서를 호스트 하는 VSPackage 필요 합니다.  
...
3. 를 [!INCLUDE[csprcs](../../includes/csprcs-md.md)] 언어로 선택 하 고 옵션을 선택 하 여 새 키 파일을 생성 합니다. **다음**을 클릭합니다.  
...
6. MyLanguageService.cs 파일에서 다음 문을 추가 합니다 `using` .  
...
1. MyLanguagePackagePackage.cs 파일을 열고 다음 문을 추가 합니다 `using` .  
...
3. 새 클래스 파일에서 다음 문을 추가 합니다 `using` .  
