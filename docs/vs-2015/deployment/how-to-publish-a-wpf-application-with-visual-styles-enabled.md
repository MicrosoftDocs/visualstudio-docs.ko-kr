...
시각적 스타일을 사용 하면 사용자가 선택한 테마에 따라 공용 컨트롤의 모양을 변경할 수 있습니다. 기본적으로 비주얼 스타일은 WPF (Windows Presentation Foundation) 응용 프로그램에 대해 사용 하도록 설정 되어 있지 않으므로 수동으로 사용 하도록 설정 해야 합니다. 그러나 WPF 응용 프로그램에 비주얼 스타일을 사용 하도록 설정 하 고 솔루션을 게시 하면 오류가 발생 합니다. 이 항목에서는 비주얼 스타일을 사용 하는 WPF 응용 프로그램을 게시 하기 위한 프로세스 및이 오류를 해결 하는 방법에 대해 설명 합니다. 비주얼 스타일에 대 한 자세한 내용은 [비주얼 스타일 개요](https://msdn.microsoft.com/5b5d7bb6-684f-478d-bf5f-b8d18bbcff2e)를 참조 하세요. 오류 메시지에 대 한 자세한 내용은 [ClickOnce 배포의 특정 오류 문제 해결](../deployment/troubleshooting-specific-errors-in-clickonce-deployments.md)을 참조 하세요.

 오류를 해결 하 고 솔루션을 게시 하려면 다음 작업을 수행 해야 합니다.

- [비주얼 스타일을 사용 하지 않고 솔루션을 게시](#BKMK_publishsolwovs)합니다.

- [매니페스트 파일을 만듭니다](#BKMK_CreateManifest).
...
## <a name="publish-the-solution-without-visual-styles-enabled"></a><a name="BKMK_publishsolwovs"></a> 비주얼 스타일을 사용 하지 않고 솔루션 게시
...
1. 프로젝트에 비주얼 스타일을 사용 하도록 설정 하지 않았는지 확인 합니다. 먼저 프로젝트의 매니페스트 파일에서 다음 XML을 확인 합니다. 그런 다음 XML이 있는 경우 주석 태그로 XML을 묶습니다.
    ...
    **Visual Basic 프로젝트에서 매니페스트 파일을 열려면**
    ...
    1. 메뉴 모음에서 **프로젝트**, _projectname_**속성**을 선택 합니다. 여기서 *ProjectName* 는 WPF 프로젝트의 이름입니다.
    ...
    **C # 프로젝트에서 매니페스트 파일을 열려면**
    ...
    3. **솔루션 탐색기**에서 **모든 파일 표시** ()를 선택 합니다.
...
## <a name="create-a-manifest-file"></a><a name="BKMK_CreateManifest"></a> 매니페스트 파일 만들기
...
## <a name="embed-the-manifest-file-into-the-executable-file-of-the-published-solution"></a><a name="BKMK_embedmanifest"></a> 게시 된 솔루션의 실행 파일에 매니페스트 파일 포함

1. **Visual Studio 명령 프롬프트**를 엽니다.

    **Visual Studio 명령 프롬프트**를 여는 방법에 대 한 자세한 내용은 [명령 프롬프트](https://msdn.microsoft.com/library/94fcf524-9045-4993-bfb2-e2d8bad44219)를 참조 하세요.
...
## <a name="sign-the-application-and-deployment-manifests"></a><a name="BKMK_signappdeplyman"></a> 응용 프로그램 및 배포 매니페스트에 서명

1. 명령 프롬프트에서 다음 명령을 실행 하 여 `.deploy` 현재 디렉터리에 있는 실행 파일에서 확장을 제거 합니다.
   ...
   > [!NOTE]
   > 이 예에서는 하나의 파일만 파일 확장명을 갖는 것으로 가정 `.deploy` 합니다. 이 디렉터리에서 파일 확장명이 인 모든 파일의 이름을 바꿔야 `.deploy` 합니다.
...
2. 명령 프롬프트에서 다음 명령을 실행 하 여 응용 프로그램 매니페스트에 서명 합니다.
   ...
   > [!NOTE]
   > 이 예제에서는 프로젝트의 파일을 사용 하 여 매니페스트에 서명 하는 것으로 가정 합니다 `.pfx` . 매니페스트에 서명 하지 않는 경우 `–cf` 에는이 예제에 사용 된 매개 변수를 생략할 수 있습니다. 암호를 요구 하는 인증서를 사용 하 여 매니페스트에 서명 하는 경우 `–password` 옵션 ()을 지정 `For example: mage –u MyWPFApp.exe.manifest –cf ..\..\..\MyWPFApp_TemporaryKey.pfx – password Password` 합니다.

3. 명령 프롬프트에서 다음 명령을 실행 하 여 `.deploy` 이 절차의 이전 단계에서 이름을 바꾼 파일의 이름에 확장명을 추가 합니다.
   ...
   > [!NOTE]
   > 이 예에서는 하나의 파일만 파일 확장명이 있다고 가정 `.deploy` 합니다. 이 디렉터리에서 이전에 파일 이름 확장명을 가진 모든 파일의 이름을 바꿔야 `.deploy` 합니다.
...
4. 명령 프롬프트에서 다음 명령을 실행 하 여 배포 매니페스트에 서명 합니다.
   ...
   > [!NOTE]
   > 이 예제에서는 프로젝트의 파일을 사용 하 여 매니페스트에 서명 하는 것으로 가정 합니다 `.pfx` . 매니페스트에 서명 하지 않는 경우 `–cf` 에는이 예제에 사용 된 매개 변수를 생략할 수 있습니다. 암호를 요구 하는 인증서를 사용 하 여 매니페스트에 서명 하는 경우 `–password` 이 예제와 같이 옵션을 지정 `For example: mage –u MyWPFApp.exe.manifest –cf ..\..\..\MyWPFApp_TemporaryKey.pfx – password Password` 합니다.
...
[ClickOnce 배포](../deployment/troubleshooting-specific-errors-in-clickonce-deployments.md) 
 의 특정 오류 문제 해결 [비주얼 스타일 개요](https://msdn.microsoft.com/5b5d7bb6-684f-478d-bf5f-b8d18bbcff2e) 
