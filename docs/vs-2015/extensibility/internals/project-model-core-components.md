...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistHierarchyItem2>|프로젝트에서 해당 프로젝트 항목의 모든 요소를 디스크의 파일 또는 다른 저장소 시스템의 개체와 함께 관리할 수 있도록 합니다. 인터페이스는 `IVsPeristHierarchyItem2` 인터페이스를 구현 하지 않는 항목에 사용 됩니다 <xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2> .|  
