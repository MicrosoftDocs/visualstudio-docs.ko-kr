...
1. Visual Studio에서 프로젝트를 연 상태에서 서버 쪽 JavaScript 파일(예: *server.js*)을 열고 왼쪽 여백의 여백을 클릭하여 중단점을 설정합니다.
    ...
    ![JavaScript 코드를 보여 주는 Visual Studio 코드 창의 스크린샷 왼쪽 제본용 여백의 빨간색 점은 중단점이 설정되어 있음을 나타냅니다.](../javascript/media/tutorial-nodejs-react-set-breakpoint.png)
...
1. Visual Studio로 전환한 다음 JavaScript 파일, TypeScript 파일 또는 JSX 파일일 수 있는 소스 코드에서 중단점을 설정합니다. (return 문 또는 var 선언과 같은 중단점을 허용하는 코드 줄에 중단점을 설정합니다.)
    ...
    ![Visual Studio 코드 창의 스크린샷 return 문이 선택되어 있고 왼쪽 제본용 여백의 빨간색 점은 중단점이 설정되어 있음을 나타냅니다.](../javascript/media/tutorial-nodejs-react-set-breakpoint-client-code.png)
