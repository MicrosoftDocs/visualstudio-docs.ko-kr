...
1. **새 프로젝트 만들기** 창에서 검색 상자에 *콘솔* 을 입력합니다. 그런 다음, 언어 목록에서 **C#** 을 선택한 다음, 플랫폼 목록에서 **Windows** 를 선택합니다. 
   ...
   ![콘솔 앱에 대한 C# 템플릿을 선택합니다.](../csharp/media/vs-2019/get-started-create-console-project.png)
...
1. **새 프로젝트 구성** 창에서 **프로젝트 이름** 상자에 *GetStartedDebugging* 을 입력합니다. 그리고 **다음** 을 선택합니다.

1. 권장되는 대상 프레임워크(.NET Core 3.1) 또는 .NET 5를 선택한 후 **만들기** 를 선택합니다.
...
## <a name="navigate-code-and-inspect-data-using-data-tips"></a>데이터 팁을 사용하여 코드 탐색 및 데이터 검사
...
1. 변수에 포함된 모든 요소를 포함하는 속성을 보려면 `letters` 변수를 확장합니다.
     ...
     ![‘name+= letters[I]’ 문이 강조 표시되고 드롭다운 목록에 문자 배열의 요소가 표시된 Visual Studio 디버거의 스크린샷](../csharp/media/get-started-view-data-tip.png)

1. 그런 다음 `name` 변수 위로 마우스를 이동하면 현재 값(빈 문자열)이 표시됩니다.

1. **F10** 키를 두 번 눌러(또는 **디버그 > 프로시저 단위 실행** 선택) `SendMessage` 메서드 호출로 이동한 다음 **F10** 키를 한 번 더 누릅니다.

     F10 키는 앱 코드의 함수 또는 메서드를 한 단계씩 실행하지 않고 디버거를 다음 문으로 진행합니다(코드는 계속 실행되고 있음). `SendMessage` 메서드 호출에서 F10 키를 눌러 `SendMessage`에 대한 구현 코드(지금은 관심이 없을 수 있음)를 건너뛰었습니다.

1. **F10** 키(또는 **디버그** > **프로시저 단위 실행**)를 몇 번 눌러 `for` 루프를 여러 번 반복하고 중단점에서 다시 일시 중지하고 매번 `name` 변수 위로 마우스를 이동하여 값을 확인합니다.

     ![디버깅 중 F10 키를 눌러 “프로시저 단위 실행”을 수행하고 루프를 반복하는 효과를 보여 주는 Visual Studio 디버거의 애니메이션 스크린샷](../csharp/media/get-started-data-tip.gif)
...
1. `Main` 메서드의 `for` 루프에서 여전히 일시 중지한 동안 `SendMessage` 메서드 호출에서 일시 중지할 때까지 **F11** 키를 누릅니다(또는 **디버그 > 한 단계식 코드 실행** 선택).
...
1. **F11** 키를 한 번 더 눌러 `SendMessage` 메서드를 한 단계씩 코드 실행합니다.
     ...
     F11 키는 **한 단계씩 코드 실행** 명령으로서 한 번에 하나의 명령문씩 앱을 실행합니다. F11 키는 실행 흐름을 가장 자세히 검사할 수 있는 좋은 방법입니다. 기본적으로 디버거는 사용자 코드가 아닌 코드를 건너뜁니다(자세한 내용은 [내 코드만](../../debugger/just-my-code.md)을 참조하세요).
...
1. **Shift** + **F11** 키(또는 **디버그 > 프로시저 나가기**)를 누릅니다.
     ...
     `SendMessage` 메서드 호출에서 일시 중지된 `Main` 메서드의 `for` 루프로 다시 이동해야 합니다. 코드를 이동하는 다양한 방법에 대한 자세한 내용은 [디버거에서 코드 탐색](../../debugger/navigating-through-code-with-the-debugger.md)을 참조하세요.
