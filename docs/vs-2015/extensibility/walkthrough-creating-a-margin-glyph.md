...
## <a name="defining-the-glyph"></a>문자 모양 정의  
 인터페이스를 구현 하 여 문자 모양을 정의 <xref:Microsoft.VisualStudio.Text.Editor.IGlyphFactory> 합니다.  
  
#### <a name="to-define-the-glyph"></a>문자 모양을 정의 하려면  
  
1. 클래스 파일을 추가하고 이름을 `TodoGlyphFactory`로 지정합니다.  
  
2. 선언을 사용 하 여 다음을 추가 합니다.  
...
## <a name="defining-a-todo-tag-and-tagger"></a>Todo 태그 및 태거 정의  
 태그 유형과 태거를 만들고 태거 공급자를 사용 하 여 내보내서 이전 단계에서 정의한 UI 요소와 표시기 여백 간의 관계를 정의 합니다.  
...
5. 짙은 파란색 윤곽선이 있는 연한 파란색 원이 코드 창의 왼쪽에 있는 표시기 여백에 표시 됩니다.
