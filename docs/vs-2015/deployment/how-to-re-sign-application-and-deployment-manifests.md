---
title: '방법: 응용 프로그램 및 배포 매니페스트 다시 서명 | Microsoft Docs'
ms.date: 11/15/2016
...
---
# <a name="how-to-re-sign-application-and-deployment-manifests"></a>방법: 애플리케이션 및 배포 매니페스트에 다시 서명
...
## <a name="re-signing-the-application-and-deployment-manifests"></a>응용 프로그램 및 배포 매니페스트에 다시 서명  
 이 절차에서는 응용 프로그램 매니페스트 파일 (.manifest)을 이미 변경한 것으로 가정 합니다. 자세한 내용은 [방법: 배포 속성 변경](https://msdn.microsoft.com/66052a3a-8127-4964-8147-2477ef5d1472)을 참조 하세요.  
...
3. 다음 명령을 입력 하 여 응용 프로그램 매니페스트 파일에 서명 합니다. ManifestFileName을 매니페스트 파일의 이름 및 확장명으로 바꿉니다. 인증서를 인증서 파일의 상대 또는 정규화 된 경로로 바꾸고 Password를 인증서의 암호로 바꿉니다.  
...
5. 필요에 따라 마스터 배포 매니페스트 (게시 \\ *appname*. 응용 프로그램)를 버전 배포 디렉터리 (publish\Application Files \\ *appname*_*버전*)에 복사 합니다.  
  
## <a name="updating-and-re-signing-the-application-and-deployment-manifests"></a>응용 프로그램 및 배포 매니페스트 업데이트 및 다시 서명  
...
4. 업데이트 된 파일에 대 한 새 해시를 사용 하 여 응용 프로그램 매니페스트를 업데이트 하 고 응용 프로그램 매니페스트 파일에 서명 하려면 다음 명령을 입력 합니다. ManifestFileName을 매니페스트 파일의 이름 및 확장명으로 바꿉니다. 인증서를 인증서 파일의 상대 또는 정규화 된 경로로 바꾸고 Password를 인증서의 암호로 바꿉니다.  
...
7. 필요에 따라 마스터 배포 매니페스트 (게시 \\ *appname*. 응용 프로그램)를 버전 배포 디렉터리 (publish\Application Files \\ *appname*_*버전*)에 복사 합니다.  
