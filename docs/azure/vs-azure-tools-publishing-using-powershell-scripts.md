---
title: PowerShell을 사용 하 여 개발 및 테스트 환경에 게시
...
Azure 개발 시 Visual Studio에서 PowerShell을 사용하기 위한 추가 도구와 리소스를 사용할 수 있습니다. [Visual Studio용 PowerShell](https://marketplace.visualstudio.com/items?itemName=AdamRDriscoll.PowerShellToolsforVisualStudio2015)을 참조하세요.

## <a name="generating-the-publish-scripts"></a>게시 스크립트 생성

새 프로젝트를 만들 때 [이 지침](/azure/virtual-machines/windows/classic/web-app-visual-studio)을 따라 웹 사이트를 호스팅하는 가상 머신의 게시 스크립트를 생성할 수 있습니다. 또한 [Azure App Service에서 웹앱용 게시 스크립트를 생성](/azure/app-service/scripts/app-service-powershell-deploy-github)할 수도 있습니다.
...
1. 프로젝트의 웹 배포 패키지를 만듭니다. 웹 배포 패키지는 웹 사이트 또는 가상 머신에 복사하려는 파일이 포함된 압축 아카이브(.zip 파일)입니다. Visual Studio에서 모든 웹 애플리케이션에 사용할 웹 배포 패키지를 만들 수 있습니다.
   ...
   자세한 내용은 [방법: Visual Studio에서 웹 배포 패키지 만들기](/previous-versions/aspnet/dd465323(v=vs.110))를 참조하세요. 또한 [게시 스크립트 사용자 지정 및 확장](#customizing-and-extending-the-publish-scripts)에서 설명하는 대로 웹 배포 패키지 생성을 자동화할 수 있습니다.
...
1. `New-WebDeployPackage`을(를) 다음 코드로 바꾸고 `$msbuildCmd` 생성 줄에서 자리 표시자를 바꿉니다. 이 코드는 Visual Studio 2019용입니다. Visual Studio 2017을 사용하는 경우 **VisualStudioVersion** 속성을 `15.0`(Visual Studio 2015는 ‘14.0’, Visual Studio 2013은 `12.0`)으로 변경하세요.
    ...
    웹 애플리케이션을 빌드하려면 MsBuild.exe를 사용합니다. 도움말은 [MSBuild Command-Line 참조](../msbuild/msbuild-command-line-reference.md) 를 참조 하세요.
...
[Windows PowerShell을 사용하여 스크립팅](/powershell/scripting/overview)을 읽어 PowerShell 스크립팅에 대해 자세히 알아보고 [스크립트 센터](https://azure.microsoft.com/documentation/scripts/)에서 기타 Azure PowerShell 스크립트에 대해 알아보세요.