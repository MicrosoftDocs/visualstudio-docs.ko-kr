...
description: TrustInfo 요소는 클라이언트 컴퓨터에서 응용 프로그램을 실행 하는 데 필요한 최소 보안 권한을 설명 합니다. TrustInfo 요소는 필수입니다.
...
## <a name="security"></a>security
 필수 사항입니다. 이 요소는 `trustInfo` 요소의 자식입니다. `applicationRequestMinimum` 요소를 포함하며 특성이 없습니다.

## <a name="applicationrequestminimum"></a>applicationRequestMinimum
 필수 사항입니다. 이 요소는 `security` 요소의 자식이며 `PermissionSet`, `assemblyRequest`및 `defaultAssemblyRequest`요소를 포함합니다. 이 요소에는 특성이 없습니다.

## <a name="permissionset"></a>PermissionSet
 필수 사항입니다. 이 요소는 `applicationRequestMinimum` 요소의 자식이며 `IPermission` 요소를 포함합니다. 이 요소에는 다음 특성이 있습니다.

- `ID`

     필수 사항입니다. 권한 집합을 알려 줍니다. 이 특성은 아무 값이나 될 수 있습니다. ID는 `defaultAssemblyRequest` 및 `assemblyRequest` 특성에서 참조됩니다.

- `version`

     필수 사항입니다. 권한의 버전을 알려 줍니다. 일반적으로 이 값은 `1`입니다.
...
- `class`
     ...
     필수 사항입니다. 강력한 이름으로 권한 클래스를 식별합니다. 예를 들어 다음 코드는 `FileDialogPermission` 유형을 알려 줍니다.
...
- `Unrestricted`
     ...
     필수 사항입니다. 애플리케이션에 이 권한을 무제한으로 부여할 필요가 있는지 식별합니다. `true`인 경우 권한 부여에 조건이 없습니다. `false`또는 이 특성이 정의되지 않은 경우에는 `IPermission` 태그에 정의된 권한별 특성에 따라 제한됩니다. 다음 권한을 사용합니다.
...
- `permissionSetReference`
     ...
     필수 사항입니다. 기본 권한인 권한 집합의 ID를 알려 줍니다. 권한 집합은 `PermissionSet` 요소에서 선언됩니다.
...
- `Name`
     ...
     필수 사항입니다. 어셈블리 이름을 알려 줍니다.

- `permissionSetReference`

     필수 사항입니다. 이 어셈블리에서 필요로 하는 권한 집합의 ID를 식별합니다. 권한 집합은 `PermissionSet` 요소에서 선언됩니다.
